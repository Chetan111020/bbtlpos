...
class Util
{
    ...
    */
     ...
     * @return array
     */
    public function payment_types($location = null, $show_advance = false, $business_id = 4)
    {
        if(!empty($location)){
        ...
        $payment_types['credit_memo'] = "Credit Memo";
        ...
        $custom_labels['payments'] = $custom_labels['payments'] ?? [];
        foreach($custom_labels['payments'] as $key=>$custom_pay){
            if($custom_pay != "" && !in_array($key,array_keys($payment_types))){
                $payment_types[$key] = $custom_pay;
            }
        }
        ...
        $payment_types = ['credit' => 'Credit'] + $payment_types;
        if ($show_advance) {
    ...
    */
    ...
    public function getInvoiceUrl($transaction_id, $business_id)
    {
        ...
        if (empty($transaction->invoice_token)) {
            ...
            $expire_at->modify('+'.$validity_days.' days');

            $transaction->invoice_token = $this->generateToken();
            $transaction->token_expire_at = $expire_at->format('Y-m-d');
            $transaction->save();
        }
    ...
    public function getCreditMemo($transaction_id, $business_id)
    {
        ...
        return route('show_credit_memo', ['token' => $transaction->invoice_token]);

        // return route('show_credit_memo', ['id' => $transaction]);

        // return $f->format($number);
    }

    public function url_for_packing($transaction_id)
    {
      $transaction = Transaction::where('transaction_id', $transaction_id)->findOrFail($transaction_id);
     // dd($transaction);
    }
    ...
    /**
     * Uploads multiple document Add to the server if present in the request
     */
    public function uploadMultipleFile($request, $file_name, $dir_name, $file_type = 'document')
    {
        $data = Null;
    ...
    /**
     * Uploads multiple document Edit to the server if present in the request
     */
    public function uploadMultipleFileUpdate($request, $file_name, $dir_name, $file_type = 'document')
    {
        $data = Null;
        $files = $request->file('documents');
        if($request->hasFile('documents'))
        {
            foreach ($files as $key =>$file) {
                if ($file && $file->isValid()) {
                    ...
                    if ($file->getSize() <= config('constants.document_size_limit')) {
                        $new_file_name = time().$key. '_' . $file->getClientOriginalName();
                        if ($file->storeAs($dir_name, $new_file_name)) {
                            $uploaded_file_name = $new_file_name;
                            $data[] =  $new_file_name;
                        }
     ...
     * @return array
     */
    public function replaceTags($business_id, $data, $transaction, $contact = null,$print="")
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{contact_address}') !== false) {
                $contact_address = empty($contact) ? $transaction->contact->address_line_1 : $contact->address_line_1;

                $data[$key] = str_replace('{contact_address}', $contact_address, $data[$key]);
            }
            ...
            //Replace invoice number
            if (strpos($value, '{invoice_number}') !== false &&  $transaction->type == 'sell_return' ) {
            ...
            if (strpos($value, '{invoice_date}') !== false ) {
                $invoice_date = $transaction->transaction_date;

                $data[$key] = str_replace('{invoice_date}', date("M d", strtotime($invoice_date)) , $data[$key]);
            }
            ...
            if (strpos($value, '{transaction_date}') !== false ) {
                ...
                $data[$key] = str_replace('{transaction_date}', date("m/d/Y", strtotime($transaction_date)) , $data[$key]);
            }
            //Replace invoice number
            if (strpos($value, '{invoice_number}') !== false && $transaction->type == 'sell') {
                $invoice_number = $transaction->type == 'sell' ? $transaction->invoice_no : '';
            ...
            if (strpos($value, '{business_logo}') !== false) {
                ...
                // echo $logo_name;
                $business_logo = !empty($logo_name) ? '<img style="width: 200px; height: 100px;" src="' . url('uploads/invoice_logos/' . $logo_name) . '" alt="Business Logo" >' : '';
            ...
            if (!empty($transaction) && strpos($value, '{invoice_url}') !== false && $transaction->type == 'sell') {
                if($print!="")
                {
                    $invoice_url = $this->getInvoiceUrlForPrint($transaction->id, $transaction->business_id);
                }
                else
                {
            ...
            if (!empty($transaction) && strpos($value, '{invoice_url}') !== false && $transaction->type == 'sell_return') {
                $invoice_url = $this->getCreditMemo($transaction->id, $transaction->business_id);
                $data[$key] = str_replace('{invoice_url}', $invoice_url, $data[$key]);
            }

            //packing slip Url pass for mails..
            if (!empty($transaction) && strpos($value, '{packing_slip}') !== false && $transaction->type == 'sell') {
                $packing_slip = $this->url_for_packing($transaction->id);
                $data[$key] = str_replace('{packing_slip}', $packing_slip, $data[$key]);
            }
            ...
            //Replace Tracking id
            if (strpos($value, '{delivered_to}') !== false) {
                $delivered_to = $transaction->type == 'sell' ?  $transaction->delivered_to : '';

                $data[$key] = str_replace('{delivered_to}', $delivered_to, $data[$key]);
            }
            if (strpos($value, '{shipping_address}') !== false) {
                $shipping_address = $transaction->type == 'sell' ?  $transaction->shipping_address : '';
                if($shipping_address == "[object Object]"){
                    $shipping_address = "--";
                }
                $data[$key] = str_replace('{shipping_address}', $shipping_address, $data[$key]);
            }
    ...
    public function replaceTagsForBalanceDue($business_id, $data, $contact,$new_fromdate,$new_todate, $print="")
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        if (!empty($contact)){
            ...
            $query = Transaction::where('transactions.contact_id', $contact->id)
                        ->where('transactions.business_id', $business_id)
                        ->where('transactions.type', '=', 'sell')
                        ->where('transactions.payment_status','!=','paid')
                        ->where('transactions.transaction_date','>=',$new_fromdate)
                        ->where('transactions.transaction_date','<=',$new_todate)
                         ->with(['contact', 'payment_lines'])->get();

            // $query;

            // $due = $query->final_total - $total_paid;
        }

        // echo "<pre>";
        // // print_r($query);
        // // foreach($contact as $data1){
        //  print_r($contact->pay_term_number) ;
        //  print_r($contact->pay_term_type) ;

        //  $currentDateTime = \Carbon::now();
        //  $newDateTime = \Carbon::now()->subDays($contact->pay_term_number);
        // echo "<br>". $newDateTime;
        ...
        foreach ($data as $key => $value) {
            ...
            foreach($query as $transaction){
                ...
                $invoice_url = $this->getInvoiceUrl($transaction_id, $business_id);
                $date = $transaction->transaction_date;

                if($pay_term_type == 'days'){
                    $pay_term_date =  \Carbon::parse($transaction->transaction_date)->addDays($pay_term_number);
                }
                if($pay_term_type == 'months'){
                    $pay_term_date =  \Carbon::parse($transaction->transaction_date)->addMonths($pay_term_number);
                }
                // date("Y-m-d", strtotime('+ '.$pay_term_number , strtotime($date)));

                // $date->addDays($pay_term_number);

                // $view = "<a href='".$invoice_url."'>View</a>";
                ...
                if (!empty($transaction)) {
                 ...
                 $due = $transaction->final_total - $total_paid;
                 $due_amount .= $this->num_f($due, true)."<br>";

                //Disable By Dhruvik Due to error in duedate $view .= PHP_EOL.'Invoice#: '.$invoice_no .PHP_EOL .'Due Date: '. $pay_term_date->format('d M Y')   .PHP_EOL .'Overdue By: '. $currentDateTime->diffInDays($pay_term_date) . PHP_EOL .'Amount: $ '. number_format((float)$due, 2, '.', '')  . PHP_EOL ;
                //  $view .= PHP_EOL.'Invoice#: '.$invoice_no .PHP_EOL .'Overdue By: '. $currentDateTime->diffInDays($pay_term_date) . PHP_EOL .'Amount: $ '. number_format((float)$due, 2, '.', '')  . PHP_EOL ;

                 $view .= PHP_EOL.'Invoice#: '.$invoice_no .'- $ '. number_format((float)$due, 2, '.', '') .', '. $currentDateTime->diffInDays($pay_term_date) .' Days Overdue ' . PHP_EOL ;
                 //$view .= 'view' .'<br>';

                // $due_date .= "<tr style='padding:5px;border: 1px solid;'><td style='text-align: center;border: 1px solid;'>".$invoice_no .'</td><td style="padding:5px;text-align: center;border: 1px solid;">'.$view .'</td></tr>';

            }


            if (strpos($value, '{due_date}') !== false) {
                    $data[$key] = str_replace('{due_date}', '<table style="padding:5px;border: 1px solid;border-collapse: collapse;"><tr style="padding:5px;border: 1px solid;"><th  style="text-align: center;border: 1px solid;"><b>Invoice No</b></th><th style="text-align: center;border: 1px solid;"><b>Invoice view</b></th></tr>'. $due_date  .'</table>' , $data[$key]);
            }

            if (strpos($value, '{view}') !== false) {
                $data[$key] = str_replace('{view}', $view, $data[$key]);
            }
            ...
            $start_date = date('m/d/Y', strtotime($new_fromdate));
            $end_date =date('m/d/Y', strtotime($new_todate));

            if (!empty($transaction_id) && strpos($value, '{start_date}') !== false) {

                $data[$key] = str_replace(array('{start_date}'), $start_date , $data[$key]);
            }
            if (!empty($transaction_id) && strpos($value, '{end_date}') !== false) {

                $data[$key] = str_replace(array('{end_date}'), $end_date , $data[$key]);
            }

            if (!empty($transaction_id) && strpos($value, '{invoice_url}') !== false) {

                $data[$key] = str_replace(array('{invoice_url}'), $view , $data[$key]);
            }

            if (strpos($value, '{invoice_no}') !== false) {
                $data[$key] = str_replace(array('{invoice_no}'),  $invoice_no , $data[$key]);
            }

            if (strpos($value, '{contact_name}') !== false) {
                $contact_name = empty($contact) ? $contact->name : $contact->name;
            ...
            if (strpos($value, '{cumulative_due_amount}') !== false) {
                $contact_payments =
                    // $this->getContactDue($contact->id);
                    Contact::where('contacts.id', $contact->id)
    ...
    */
    ...
    //added by developer1
    public function getInvoiceUrlForPrint($transaction_id, $business_id)
    {
        $transaction = Transaction::where('business_id', $business_id)
                            ->findOrFail($transaction_id);
        if (empty($transaction->invoice_token)) {
        ...
        return route('show_print_invoice', ['token' => $transaction->invoice_token , 'print' => 'print']);
    }
}