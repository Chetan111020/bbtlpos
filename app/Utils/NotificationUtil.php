...
class NotificationUtil extends Util
{
    ...
    public function SendEmailNotification($business_id, $notification_type, $transaction, $email,$print="", $invoice_show_type="")
    {
        $status = 0;
        $notification_template = NotificationTemplate::where('business_id', $business_id)
                ->where('template_for', $notification_type)
                ->first();
            /*echo "<pre>";
            print_r($notification_template);
            echo "</pre>";
            exit; */
        $location = BusinessLocation::where('business_locations.business_id', 4)->first();
        if(!empty($location->email))
        {
            $data['bcc'] = $location->email;
        }
        $business = Business::findOrFail($business_id);
        $data['email_settings'] = $business->email_settings;
        $data['sms_settings'] = $business->sms_settings;
        ...
        if (!empty($notification_template))
        {
                ...
                $tag_replaced_data = $this->replaceTags($business_id, $orig_data, $transaction,null,$print);
                ...
                if (!empty($email)) {
                    ...
                    // for PDF
                    if(!empty($transaction->pdf_invoice)){
                        $data['attachment'] = $transaction->pdf_invoice;
                        $data['attachment_name'] = "Invoice #".$transaction->invoice_no.'.pdf';
                    }
                    ...
                    if (array_key_exists($notification_type, $customer_notifications)) {
                        ...
                        /*if($invoice_show_type == 'packing_slip')
                        {
                            $data['email_settings']['mail_username'] = 'invoice.notify.email@gmail.com';
                            $data['email_settings']['mail_password'] = 'Dhruvik!23456';
                            $data['email_settings']['mail_from_address'] = 'invoice.notify.email@gmail.com';
                        }*/
                        Notification::route('mail', $data['to_email'])
                                        ->notify(new CustomerNotification($data));
                        $status = 1;
