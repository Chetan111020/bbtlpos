...
class ModuleUtil extends Util
{
    ...
    public function Userlog($type,$user_id,$update_user_id,$business_id,$pro='')
    {
        $user = auth()->user();
        $message = rtrim($pro, ',');
        ...
        $log = new Userlog();
        $log->user_id = $user_id;
        $log->update_user_id = $update_user_id;
        $log->description = $type;
        $log->message = $change_message;
        $log->save();
    }
    public function UpdateUserLog($user_data, $business_id, $user_id, $id, $roles_id)
    {
        $pro='';
        $user = User::find($id);
        ...
        $roles = Role::where('name', $user->getRoleNames())->first();
        ...
        if(isset($user->id)){
            ...
            if($user->surname != $user_data['surname']){
                $pro .= 'SurName ('.$user->surname.' --> '.$user_data['surname'].'), ';
            }
            
            if(isset($role->id)){
                if($roles->name != $role->name){
                    $pro .= 'Role ('.$roles->name.' --> '.$role->name.'), ';
                }
            }
                
            if (!empty($user_data['first_name']) && $user->first_name != $user_data['first_name']) {
                $pro .= 'First Name (' . $user->first_name . ' --> ' . $user_data['first_name'] . '), ';
            }

            if($user->last_name != $user_data['last_name']){
                $pro .= 'Last Name ('.$user->last_name.' --> '.$user_data['last_name'].'), ';
            }

            if($user->email != $user_data['email']){
                $pro .= 'Email ('.$user->email.' --> '.$user_data['email'].'), ';
            }
           
            if($user->selected_contacts != $user_data['selected_contacts']){
                $pro .= 'Selected Contacts ('.$user->selected_contacts.' --> '.$user_data['selected_contacts'].'), ';
            }

            if($user->marital_status != $user_data['marital_status']){
                $pro .= 'Marital Status ('.$user->marital_status.' --> '.$user_data['marital_status'].'), ';
            }

            if($user->blood_group != $user_data['blood_group']){
                $pro .= 'Blood Group ('.$user->blood_group.' --> '.$user_data['blood_group'].'), ';
            }

            if($user->contact_number != $user_data['contact_number']){
                $pro .= 'Contact Number ('.$user->contact_number.' --> '.$user_data['contact_number'].'), ';
            }

            if($user->fb_link != $user_data['fb_link']){
                $pro .= 'Fb Link ('.$user->fb_link.' --> '.$user_data['fb_link'].'), ';
            }

            if($user->twitter_link != $user_data['twitter_link']){
                $pro .= 'Twitter Link ('.$user->twitter_link.' --> '.$user_data['twitter_link'].'), ';
            }

            if($user->social_media_1 != $user_data['social_media_1']){
                $pro .= 'Social Media 1 ('.$user->social_media_1.' --> '.$user_data['social_media_1'].'), ';
            }

            if($user->social_media_2 != $user_data['social_media_2']){
                $pro .= 'Social Media 2 ('.$user->social_media_2.' --> '.$user_data['social_media_2'].'), ';
            }

            if($user->permanent_address != $user_data['permanent_address']){
                $pro .= 'Permanent Address ('.$user->permanent_address.' --> '.$user_data['permanent_address'].'), ';
            }

            if($user->current_address != $user_data['current_address']){
                $pro .= 'Current Address ('.$user->current_address.' --> '.$user_data['current_address'].'), ';
            }

            if($user->guardian_name != $user_data['guardian_name']){
                $pro .= 'Guardian Name ('.$user->guardian_name.' --> '.$user_data['guardian_name'].'), ';
            }

            if($user->custom_field_1 != $user_data['custom_field_1']){
                $pro .= 'Custom Field 1 ('.$user->custom_field_1.' --> '.$user_data['custom_field_1'].'), ';
            }

            if($user->custom_field_2 != $user_data['custom_field_2']){
                $pro .= 'Custom Field 2 ('.$user->custom_field_2.' --> '.$user_data['custom_field_2'].'), ';
            }

            if($user->custom_field_3 != $user_data['custom_field_3']){
                $pro .= 'Custom Field 3 ('.$user->custom_field_3.' --> '.$user_data['custom_field_3'].'), ';
            }

            if($user->custom_field_4 != $user_data['custom_field_4']){
                $pro .= 'Custom Field 4 ('.$user->custom_field_4.' --> '.$user_data['custom_field_4'].'), ';
            }

            if($user->id_proof_name != $user_data['id_proof_name']){
                $pro .= 'Id Proof Name ('.$user->id_proof_name.' --> '.$user_data['id_proof_name'].'), ';
            }

            if($user->id_proof_number != $user_data['id_proof_number']){
                $pro .= 'Id Proof Number ('.$user->id_proof_number.' --> '.$user_data['id_proof_number'].'), ';
            }

            if($user->cmmsn_percent != $user_data['cmmsn_percent']){
                $pro .= 'Cmmsn Percent ('.$user->cmmsn_percent.' --> '.$user_data['cmmsn_percent'].'), ';
            }

            if($user->gender != $user_data['gender']){
                $pro .= 'Gender ('.$user->gender.' --> '.$user_data['gender'].'), ';
            }

            if($user->max_sales_discount_percent != $user_data['max_sales_discount_percent']){
                $pro .= 'Max Sales Discount Percent ('.$user->max_sales_discount_percent.' --> '.$user_data['max_sales_discount_percent'].'), ';
            }

            if($user->status != $user_data['status']){
                $pro .= 'Status ('.$user->status.' --> '.$user_data['status'].'), ';
            }

            if($user->bank_details != $user_data['bank_details']){
                $pro .= 'Bank Details ('.$user->bank_details.' --> '.$user_data['bank_details'].'), ';
            }
        }
    
        if($pro!="")
        {
            $this->Userlog('edited',$user_id,$update_user_id,$business_id,$pro);
        }
    }
}
