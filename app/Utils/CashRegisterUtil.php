        ...
        if ($status_before == 'draft' && $transaction->status == 'final') {
            ...
            $prev_payments = CashRegisterTransaction::where('transaction_id', $transaction->id)
                            ->select(
                                ...
                                DB::raw("SUM(IF(pay_method='credit_memo', IF(type='credit', amount, -1 * amount), 0)) as total_credit_memo"),
                                ...
                                DB::raw("SUM(IF(pay_method='custom_pay_8', IF(type='credit', amount, -1 * amount), 0)) as total_custom_pay_8"),
                                DB::raw("SUM(IF(pay_method='custom_pay_9', IF(type='credit', amount, -1 * amount), 0)) as total_custom_pay_9"),
            ...
            if (!empty($prev_payments)) {
                $payment_diffs = [
                    ...
                    'credit_memo' => $prev_payments->total_credit_memo,
                    ...
                    'custom_pay_8' => $prev_payments->total_custom_pay_8,
                    'custom_pay_9' => $prev_payments->total_custom_pay_9,
