...
function getTotalSold($variation_id){
    $productUtil = new ProductUtil();
    $business_id = 4;
    $arr = array('total_sold' => 0, 'total_adjusted' => 0);
    $stock_history = $productUtil->getVariationStockHistory($business_id, $variation_id, 4, null);
    if(count($stock_history)){
        ...
        $arr['total_sold'] = $total_sold;
        $arr['total_adjusted'] = $adjustment_stock;
        return $arr;
    }

}

function getCreditBalance($contact_id) {
    $credit_amount = Transaction::where('type','sell_return')->where('contact_id',$contact_id)->sum('final_total');
    ...
    return $credit_amount - $paid_credit;
}
