...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('delivery.exclusive') && auth()->user()->id != 6) {
                $menu->url(action('OrderDeliveryController@index'), __('Order Delivery'), ['icon' => 'fa fas fa-list-alt', 'active' => request()->segment(1) == 'order-delivery'])->order(80);
                return true;
            }
                ...
                $menu->dropdown(
                   ...
                   //     if (auth()->user()->can('sell.view')) {
                    ...
                    //    }
                        ...
                        //         __('lang_v1.list_payment_verified'),
                        //         ['icon' => 'fa fas fa-pen-square', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'payment-verified']
                        //     );
                        // }
                        ...
                        // $sub->url(
                        //     action('DocumentAndNoteController@reports'),
                        //     __('PO'),
                        //     ['icon' => 'fa fas fa-users', 'class' => 'docs_and_notes_btn', 'active' => request()->segment(3) == 'reports']
                        // );
            ...
            if (in_array('stock_adjustment', $enabled_modules) && ( auth()->user()->can('purchase_return.view') || auth()->user()->can('purchase_return.view.own') || auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('purchase_return.view') || auth()->user()->can('purchase_return.view.own')) {
                            $sub->url(
                                action('PurchaseReturnController@index'),
                                __('Vendor Credit Memo'),
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'purchase-return']
                            );
                        }
                        // $sub->url(
                        //     action('SellReturnController@sellReturn'),
                        //     __('Sell Return'),
                        //     ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'sells-return']
                        // );
            ...
            if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id)
                ...
                || auth()->user()->hasRole('Cashier - Level 2 (Advanced)#' . auth()->user()->business_id)
                || auth()->user()->hasRole('Accounts#' . auth()->user()->business_id)
                || in_array('account', $enabled_modules)) {

                $menu->dropdown(
                    __('Reports'),
                    function ($menu) use ($enabled_modules){
                        if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id)
                            || auth()->user()->hasRole('Administration#' . auth()->user()->business_id)
                            || auth()->user()->hasRole('Accounts#' . auth()->user()->business_id)
                            || auth()->user()->hasRole('Cashier - Level 2 (Advanced)#' . auth()->user()->business_id)) {
                            ...
                            __('Account Reports'),
                                ...
                                function ($sub)  {
                                    ...
                                    || auth()->user()->hasRole('Administration#' . auth()->user()->business_id)) {
                                        ...
                                        $sub->url(
                                            action('ReportController@getCollectedPaymentReport'),
                                            __('Collected Payment Report'),
                                            ['icon' => 'fa fas fa-search-dollar', 'active' => request()->segment(2) == 'collected-payment-report']
                                        );
                                        $sub->url(
                                            action('ReportController@NewBalanceReport'),
                                            'New Balance Report',
                                            ['icon' => 'fa fas fa-search-minus', 'active' => request()->segment(2) == 'BalanceReport']
                                        );
                                    }

                                    if(auth()->user()->hasRole('Cashier - Level 2 (Advanced)#' . auth()->user()->business_id)){
                                          $sub->url(
                                            action('ReportController@NewBalanceReport'),
                                            'New Balance Report',
                                            ['icon' => 'fa fas fa-search-minus', 'active' => request()->segment(2) == 'BalanceReport']
                                    ...
                                    if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id)
                                        ...
                                        // );
                                        $sub->url(
                                            action('ReportController@getBalanceReport'),
                                            'Balance Report',
                                            ['icon' => 'fa fas fa-search-minus', 'active' => request()->segment(2) == 'balance-report']
                                        );
                                        $sub->url(
                                        ...
                                        // }
                                        // $sub->url(
                                        //     action('ReportController@getExpiredDocumentAndNoteReport'),
                                        //     __('Expired Document & Note Report'),
                                        //     ['icon' => 'fa fas fa-address-book', 'active' => request()->segment(2) == 'expired-document-and-note-report']
                                        // );
                                        // payables and Receivables report
                                        $sub->url(
                                            action('ReportController@getPayablesReceivablesReport'),
                                            __('report.payables_receivables_report'),
                                            ['icon' => 'fa fas fa-address-book', 'active' => request()->segment(2) == 'payables-receivables-report']
                                        );
                        ...
                        if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id) || auth()->user()->hasRole('Administration#' . auth()->user()->business_id)){
                            ...
                            __(' Sales Reports'),
                                ...
                                function ($sub) use ($enabled_modules) {
                                    if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id) || auth()->user()->hasRole('Administration#' . auth()->user()->business_id)){
                                            $sub->url(
                                                action('__Compute\AnalyticsController@productAnalytics'),
                                                __('Product Analytics Report'),
                                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'category-wise-sale-report']
                                            );
                                            $sub->url(
                                                action('ReportController@getCategorywisesalereport'),
                                                __('Category Wise Sale Report'),
                                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'category-wise-sale-report']
                                            );
                                            $sub->url(
                                                action('ReportController@getBrandwisesalereport'),
                                                __('Brand Wise Sale Report'),
                                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'brand-wise-sale-report']
                                            );
                                            $sub->url(
                                                action('ReportController@getProductwisesalereport'),
                                                __('Product Wise Sale Report'),
                                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'product-wise-sale-report']
                                            );
                                            $sub->url(
                                                action('ReportController@getsaleTreeReport'),
                                                __('Sales Tree Report'),
                                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'sales-tree-report']
                                            );

                                            $sub->url(
                                                action('ReportController@productSellingReport'),
                                                __('Product Selling Report'),
                                                ['icon' => 'fa fas fa-arrow-circle-down', 'active' => request()->segment(2) == 'product-selling-report']
                                            );

                                            if (auth()->user()->can('profit_loss_report.view')) {
                                                $sub->url(
                                                    action('__Compute\ComputeController@backOrderReport'),
                                                    __('Back Order Report'),
                                                    ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'back-order']
                                                );
                                            }

                                            // if (auth()->user()->can('weekend_report.view')) {
                                                $sub->url(
                                                    action('ReportController@getWeekendReport'),
                                                    __('End Of Week Report'),
                                                    ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'weekend-report']
                                                );
                                            // }
                                            //if (auth()->user()->can('contacts_report.view')) {
                                                $sub->url(
                                                    action('ReportController@getSalesAccountRepresentativeReport'),
                                                    __('lang_v1.sales_account_representative'),
                                                    ['icon' => 'fa fas fa-users', 'active' => request()->segment(2) == 'sales-by-account-representative']
                                                );
                                            //}

                                            //export invoice tab start
                                            //if (auth()->user()->can('export_invoice.view')) {
                                                $sub->url(
                                                    action('SellPosController@exportInvoice'),
                                                    __('report.export_invoice'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'exportInvoice']
                                                );
                                           // }
                                            //export invoice tab end

                                            //echeck generate tab start
                                            // if (auth()->user()->can('echeck-generator.view')) {
                                                $sub->url(
                                                    action('SellPosController@eCheckGenerate'),
                                                    __('Generate Echeck'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'echeck-generator']
                                                );
                                            //}
                                            //deleted generate tab end
                                    }
                                },
                        ...
                        if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id)
                            ...
                            __(' Inventory Reports'),
                                ...
                                function ($sub) use ($enabled_modules) {
                                    ...
                                    || auth()->user()->hasRole('Accounts#' . auth()->user()->business_id) ) {
                                        $sub->url(
                                            action('__Compute\ReportComputeController@priceUpdateReport'),
                                            __('Price Update Report'), [
                                                'icon' => 'fa fas fa-user-secret',
                                                'active' => request()->segment(2) == 'price-update',
                                            ]
                                        );
                                        $sub->url(
                                            action('__Compute\ReportComputeController@underStockReport'),
                                            __('Under Stock Report'), [
                                                'icon' => 'fa fas fa-user-secret',
                                                'active' => request()->segment(2) == 'understock',
                                            ]
                                        );
                                        $sub->url(
                                            action('__Compute\ReportComputeController@overStockReport'),
                                            __('Over Stock Report'), [
                                                'icon' => 'fa fas fa-user-secret',
                                                'active' => request()->segment(2) == 'overstock',
                                            ]
                                        );
                                        // $sub->url(
                                        //     action('__Compute\ReportComputeController@velocityReport'),
                                        //     __('Velocity Report'), [
                                        //         'icon' => 'fa fas fa-user-secret',
                                        //         'active' => request()->segment(2) == 'velocity',
                                        //     ]
                                        // );
                                        // $sub->url(
                                        //     action('__Compute\ReportComputeController@skuReport'),
                                        //     __('SKU Report'), [
                                        //         'icon' => 'fa fas fa-user-secret',
                                        //         'active' => request()->segment(2) == 'sku-performance',
                                        //     ]
                                        // );
                                        // $sub->url(
                                        //     action('ReportController@productOnhandList'),
                                        //     __('restaurant.onhand_item_report'), [
                                        //         'icon' => 'fa fas fa-user-secret',
                                        //         'active' => request()->segment(2) == 'on-hand-reports',
                                        //     ]
                                        // );
                                        $sub->url(
                                            action('ReportController@itemInventory'),
                                            __('restaurant.item_inventory_report'), [
                                                'icon' => 'fa fas fa-user-secret',
                                                'active' => request()->segment(2) == 'item-inventory',
                                            ]
                                        );
                                        ...
                                        $sub->url(
                                            action('ReportController@getOutOfStockReport'),
                                            __('Out Of Stock Report'),
                                            ['icon' => 'fa fas fa-sliders-h', 'active' => request()->segment(2) == 'out-of-stock-report']
                                        );
                                    ...
                                    || auth()->user()->hasRole('Administration#' . auth()->user()->business_id)){
                                        if (auth()->user()->can('stock_report.view')) {
                                            $sub->url(
                                                action('ReportController@getStockReportNew'),
                                    ...
                                    //Daily items reports tab start
                                    if (auth()->user()->can('daily_items_report.view')) {
                                        $sub->url(
                                            action('ReportController@getDaliyItemsReport'),
                                            __('report.daily_items_report'),
                                            ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'daily-items-report']
                                        );
                                    }
                                    //Daily items reports tab end
                                },
                            ...
                            __(' MSA  Reports'),
                                ...
                                function ($sub) {
                                    ...
                                    //     || auth()->user()->hasRole('Accounts#' . auth()->user()->business_id) ) {
                                        ...
                                        $sub->url(
                                            action('ReportController@getjuulSellReport'),
                                            __('Juul Sell Report'),
                                                ['icon' => 'fa fas fa-arrow-circle-up', 'active' => request()->segment(2) == 'Juul-sell-report']
                                        );

                                        $sub->url(
                                            action('ReportController@getNonjuulSellReport'),
                                            __('NON Juul MSA Report'),
                                                ['icon' => 'fa fas fa-arrow-circle-up', 'active' => request()->segment(2) == 'Non-Juul-Msa-report']
                                        );
                                    ...
                                    if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id) || auth()->user()->hasRole('Administration#' . auth()->user()->business_id)) {
                                        ...
                                        if (auth()->user()->can('contacts_report.view')) {
                                            ...
                                            $sub->url(
                                             ...
                                             $sub->url(
                                                action('ReportController@getStaleCustomer'),
                                                __('lang_v1.stale_customer'),
                                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(2) == 'stale-customer']
                                            );
                                        }
                                    }
                                },
                            ...
                            __(' Deleted Reports'),
                                ...
                                function ($sub) use ($enabled_modules) {
                                    if (auth()->user()->hasRole('Admin#' . auth()->user()->business_id) || auth()->user()->hasRole('Administration#' . auth()->user()->business_id)){
                                            ...
                                            //deleted invoices tab start
                                            $sub->url(
                                                action('ReportController@DeletedInvoicesReport'),
                                                __('report.deleted_invoice'),
                                                ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-invoices-report']
                                            );
                                            //deleted invoices tab end

                                            //deleted transactions payments tab start
                                                $sub->url(
                                                    action('ReportController@DeletedTransactionPaymentsReport'),
                                                    __('report.deleted_transaction_payments'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-transaction-payments-report']
                                                );
                                            //deleted transactions payments tab

                                            //deleted credit memo tab start
                                                $sub->url(
                                                    action('ReportController@DeletedCreditmemoReport'),
                                                    __('report.deleted_creditmemo'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-creditmemo-report']
                                                );
                                            //deleted credit memo tab end

                                            //deleted vendor credit memo tab start
                                                $sub->url(
                                                    action('ReportController@DeletedVendorCreditmemoReport'),
                                                    __('report.deleted_vendorcreditmemo'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-vendorcreditmemo-report']
                                                );
                                            //deleted vendor credit memo tab end

                                            //deleted expenses tab start
                                                // $sub->url(
                                                //     action('ReportController@DeletedExpensesReport'),
                                                //     __('report.deleted_expenses'),
                                                //     ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-expenses-report']
                                                // );
                                            //deleted expenses tab end

                                            //deleted purchases tab start
                                                $sub->url(
                                                    action('ReportController@DeletedPurchasesReport'),
                                                    __('report.deleted_purchases'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-purchases-report']
                                                );
                                            //deleted purchases tab end

                                            //deleted purchases order tab start
                                                $sub->url(
                                                    action('ReportController@DeletedPurchasesOrderReport'),
                                                    __('report.deleted_purchases_order'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-purchases-order-report']
                                                );
                                            //deleted purchases order tab end

                                            //deleted Stock Adjustments tab start
                                                $sub->url(
                                                    action('ReportController@DeletedStockAdjustmentsReport'),
                                                    __('report.deleted_stock_adjustments'),
                                                    ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'deleted-stock-adjustments-report']
                                                );
                                            //deleted Stock Adjustments tab end
                                    }
                                },
                        ...
                        // deleted reports menu end

                        if(auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)){
                            $menu->url(
                                action('ReportController@nhVapeReport'), __('restaurant.nh_vape_report'), [
                                                                                    'icon' => 'fa fas fa-user-secret',
                                                                                    'active' => request()->segment(2) == 'nh-vape-report',
                                                                                ]
                            );
                        }
                        if(auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)){
                            $menu->url(
                                action('ReportController@nhVapeTransactionReport'), __('restaurant.nh_vape_transacation_report'), [
                                                                                    'icon' => 'fa fas fa-user-secret',
                                                                                    'active' => request()->segment(2) == 'nh-vape-transaction-report',
                                                                                ]
                            );
                        }
                        if(auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)){
                            $menu->url(
                                action('ReportController@ctVapeTransactionReport'), __('restaurant.ct_vape_transacation_report'), [
                                                                                    'icon' => 'fa fas fa-user-secret',
                                                                                    'active' => request()->segment(2) == 'ct-vape-transaction-report',
                                                                                ]
                            );
                        }
                        if(auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)){
                            $menu->url(
                                action('ReportController@transactionReport'), __('restaurant.transaction_report'), [
                                                                                    'icon' => 'fa fas fa-user-secret',
                                                                                    'active' => request()->segment(2) == 'ma-vape-report',
                                                                                ]
                            );
                        }
                        if(auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)){
                            $menu->url(
                                action('ReportController@paVapeTransactionReport'), __('restaurant.pa_vape_transacation_report'), [
                                                                                    'icon' => 'fa fas fa-user-secret',
                                                                                    'active' => request()->segment(2) == 'pa-vape-transaction-report',
                                                                                ]
                            );
                        }
                        if(auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)){
                            $menu->url(
                                action('ReportController@getAllTaxReport'), __('State Wise Tax Report'), [
                                                                                    'icon' => 'fa fas fa-user-secret',
                                                                                    'active' => request()->segment(2) == 'all-tax-report',
                                                                                ]
                            );
                        }
            ...
            //         {
            //             $sub->url(
            //                 action('ReportController@productOnhandList'), __('restaurant.onhand_item_report'), [
            //                                                                 'icon' => 'fa fas fa-user-secret',
            //                                                                 'active' => request()->segment(2) == 'on-hand-reports',
            //                                                             ]
            //             );
            ...
            //         {
            //             $sub->url(
            //                 action('ReportController@itemInventory'), __('restaurant.item_inventory_report'), [
            //                                                                     'icon' => 'fa fas fa-user-secret',
            //                                                                     'active' => request()->segment(2) == 'item-inventory',
            //                                                                 ]
            //             );
            //         }
            //         if (auth()->user()->can('sales_tree_report.view')) {
            //             $sub->url(
            //                 action('ReportController@getsaleTreeReport'),
            //                 __('Sales Tree Report'),
            //                 ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'sales-tree-report']
            ...
            //             );
            //         }
            //             if (auth()->user()->can('category_wise_sale_report.view')) {
            //                 $sub->url(
            //                     action('ReportController@getCategorywisesalereport'),
            //                     __('Category Wise Sale Report'),
            //                     ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'category-wise-sale-report']
            //                 );
            //             }
            //             if (auth()->user()->can('brand_wise_sale_report.view')) {
            //                 $sub->url(
            //                     action('ReportController@getBrandwisesalereport'),
            //                     __('Brand Wise Sale Report'),
            //                     ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'brand-wise-sale-report']
            //                 );
            //             }
            //             if (auth()->user()->can('product_wise_sale_report.view')) {
            //                 $sub->url(
            //                     action('ReportController@getProductwisesalereport'),
            //                     __('Product Wise Sale Report'),
            //                     ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'product-wise-sale-report']
            //                 );
            //             }
            //             if (auth()->user()->can('expired_document_and_note_report.view')) {
            //                 $sub->url(
            //                     action('ReportController@getExpiredDocumentAndNoteReport'),
            //                     __('Expired Document & Note Report'),
            //                     ['icon' => 'fa fas fa-address-book', 'active' => request()->segment(2) == 'expired-document-and-note-report']
            ...
            // smart crm menu
            if ((File::exists(base_path('Modules/SmartCRM')) && File::isDirectory(base_path('Modules/SmartCRM'))) && auth()->user()->can('smartcrm.view')) {
                $menu->dropdown(
                    __('CRM'),
                    function ($sub) {
                         $sub->url(action('\Modules\SmartCRM\Http\Controllers\SmartCRMController@index'),
                            "CRM Dashboard",
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'smart-crm' && request()->segment(2) == null]
                        );

                        $sub->url(
                            route('smartcrm.followup.index'),
                            "Follow Up",
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'follow-up' && request()->segment(3) == null]
                        );
                         $sub->url(action('ReportController@FollowUpgetStaleCustomer'),
                            "Stale Customer Follow Up",
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'followup-stale-customer' && request()->segment(3) == null]
                        );
                        $sub->url(
                            action('ReportController@CustomerAnalytics'),
                            "Customer Analytics",
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'customer-analytics' && request()->segment(3) == null]
                        );
                        $sub->url(
                            route('smartcrm.lead.Leads'),
                            "Leads",
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'leads' && request()->segment(3) == null]
                        );
                    },
                    ['icon' => 'fa fas fa-users']
                )->order(59);
            ...
            //Service Staff menu
             if (in_array('service_staff', $enabled_modules)) {
                 $menu->url(action('Restaurant\OrderController@index'), __('Order Queue'), ['icon' => 'fa fas fa-list-alt', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'orders'])->order(75);
             }

            if (auth()->user()->can('delivery.view') || auth()->user()->can('delivery.create')) {
                $menu->url(action('OrderDeliveryController@index'), __('Order Delivery'), ['icon' => 'fa fas fa-list-alt', 'active' => request()->segment(1) == 'order-delivery'])->order(80);
            }
