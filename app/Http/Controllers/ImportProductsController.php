...
class ImportProductsController extends Controller
{
        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (!empty($image_name)) {
                        $product_array['image'] = '/uploads/img/'.$image_name;
                        $product_array['main_image'] = '/img/'.$image_name;
                    } else {
                        $product_array['image'] = '';
    ...
    // Jadoo Products Import
    public function importJadooProducts(Request $request)
    {
        try {
            // Set maximum php execution time
            // ini_set('max_execution_time', 0);
            ...
            if ($request->hasFile('jadoo_products_file')) {
                $file = $request->file('jadoo_products_file');
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    $jadooProduct = JadooProduct::where('name', $value[0])->where('jadoo_name', $value[1])
                                        ->first();
                    if (!empty($jadooProduct)) {
                        if(strtolower($jadooProduct->name) == strtolower($value[0]) && strtolower($jadooProduct->jadoo_name) == strtolower($value[1]))
                        {
                            \Log::emergency("Match:" .$value[0]."--------".$value[1]."--------".$value[2]."--------".$value[3]);
                            continue;
                        }
                    }
                    JadooProduct::updateOrCreate(['name' => $value[0],'jadoo_name' => $value[1],'barcode' => $value[2],'itemcode' => $value[3]]);
                }
                DB::commit();
            }
            $output = ['success' => 1,
                            'msg' => __('Jadoo Products Imported Successfully')
                        ];
            //$message = 'Jadoo Products Imported Successfully';
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
            ...
            return redirect()
                        ->action('ProductController@jadooCreateSave')
                        ->with('status', $output);
        }
