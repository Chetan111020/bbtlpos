...
class OpeningStockController extends Controller
{
    ...
    public function addStock($opening_stocks, $location_id, $product_id)
    { 
        // if (!auth()->user()->can('product.opening_stock')) {
        //     abort(403, 'Unauthorized action.');
        // }
        ...
        try {
            ...
            if (!empty($product) && $product->enable_stock == 1) {
                ...
                $transaction_date = Session::get("financial_year.start");
                        ...
                        foreach ($value as $k => $rows) {
                            foreach ($rows as $key => $v) {
                                $purchase_price = $this->productUtil->num_uf(trim($v['purchase_price']));
                                 // $purchase_price = $this->productUtil->num_uf(trim($purchase_price));
                                ...
                                if (!is_null($purchase_line)) {
                                    ...
                                    if(isset($v['barcode'])){
                                        $purchase_line->barcode =  $v['barcode'];
                                        $purchase_line->aisle =  $v['aisle'];
                                        $purchase_line->rack =  $v['rack'];
                                        $purchase_line->shelf =  $v['shelf'];
                                        $purchase_line->bin =  $v['bin'];
                                    }
