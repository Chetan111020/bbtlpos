...
class Transaction extends Model
{
    ...
    public function po_lines()
    {
        return $this->hasMany(\App\PoLine::class);
    }
    ...
    /* find new adjustment  */
    public function getNewAdjust($id)
    {
       return $transaction = Transaction::where('type', 'stock_adjustment')->where('adjustment_type', 'abnormal')->where('id', $id)->first();
    }

    public function recalculate_old(){
        $sell_total = 0;
        $tax_total = 0;
        foreach($this->sell_lines()->get() as $sell_line){
            $unit_price = $sell_line->unit_price;
            $qunatity = $sell_line->quantity;
            $tax_total += $sell_line->pos_line_tax_amount;
            $sell_total += round(($unit_price * $qunatity), 2);
        }
        $this->total_before_tax = $sell_total;
        $this->final_total = round($sell_total + $tax_total + $this->shipping_charges - $this->discount_amount, 2);
        $this->save();
    }
    ...
    public function recalculate(){
        ...
        foreach($this->sell_lines()->get() as $sell_line){
            ...
            if($sell_line->quantity == 0){
                $sell_line->pos_line_tax_amount = 0;
                $sell_line->save();
            }
        }

        $this->total_before_tax = $sell_total;
        $net_total = round($sell_total + $tax_total, 2);

        $discount_amount = round($this->discount_amount, 2);
        if($this->discount_type == 'percentage' && $discount_amount > 0){
            $discount_amount = round(($discount_amount * $net_total) / 100, 2);
        }

        $this->final_total = round($net_total + $this->shipping_charges - $discount_amount, 2);
        $this->save();
    }
    ...
    public function getDeliveryMethod(){
        ...
        return $delivery_methods[$this->delivery_method] ?? $this->delivery_method;
    }
}
