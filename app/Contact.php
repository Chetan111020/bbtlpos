...
// implements Auditable
{
    ...
    */
    ...
    public function selling_price_groups()
    {
        return $this->belongsTo(\App\SellingPriceGroup::class, 'customer_group_id');
    }
...
/*
  ...
  */      return $query;
     ...
     * @return array users
     */
    public static function customersDropdown($business_id, $prepend_none = true, $append_id = true, $withBusinessName = false)
    {
        $all_contacts = Contact::where('business_id', $business_id)
        ...
        if($withBusinessName){
            if ($append_id) {
                $all_contacts->select(
                    DB::raw("IF(contact_id IS NULL OR contact_id='', CONCAT(supplier_business_name,' - ',name), CONCAT(supplier_business_name,' - ',name, ' (', contact_id, ')')) AS customer"),
                    'id'
                    );
            } else {
                $all_contacts->select('id', DB::raw("CONCAT(supplier_business_name,' - ',name) as customer"));
            }
        }
        else{
            if ($append_id) {
     ...
     * @return array users
     */
    public static function customersCompanyDropdown($business_id, $prepend_none = true, $append_id = true, $sales_rep_strict_check = false)
    {
        $all_contacts = Contact::where('business_id', $business_id)
                        ->whereIn('type', ['customer', 'both'])
                        ->active();
        ...
        // if ($append_id) {
            $all_contacts->select(
                DB::raw("IF(supplier_business_name IS NULL OR supplier_business_name='', name, CONCAT(supplier_business_name, ' - ', name)) AS customer"),
                'id'
                );
        // } else {
        //     $all_contacts->select('id', DB::raw("supplier_business_name as customer"));
        // }
        ...
        if($sales_rep_strict_check){
            $all_contacts->where('contacts.sales_rep', auth()->user()->id);
        }
    ...
    public static function getOwnCustomers($business_id, $prepend_none = true, $append_id = true, $sales_rep_strict_check = false)
    {
        $all_contacts = Contact::where('business_id', $business_id)
                        ->whereIn('type', ['customer', 'both'])
                        ->active();
