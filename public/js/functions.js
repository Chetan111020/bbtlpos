...
function __add_percent(amount, percentage) {
    var amount = parseFloat(amount);
    // console.log(amount);
    ...
    if(percentage == 100)
        var selling_price=amount/(0.01/100);
    else
        var selling_price=amount/((100-percentage)/100);
...
function __substract_percent(amount, percentage = 0) {
    ...
    // const selling_price=percentage/((100-amount)/100);
    // return selling_price;
...
function __get_rate(principal, amount) {
    ...
    // return Decimal.mul(div, 100).toNumber();
     var rate = (1 - (principal / amount))*100;
    return rate;
}
