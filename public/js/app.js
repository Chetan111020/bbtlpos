$(document).ready(function() {
    ...
    //     if($(this).attr('href') != '#'){
    //         $('.gs_bar').append(new Option($(this).find('span').html(), $(this).attr('href')));
    //     }
    // });
    // setTimeout(() => {
    //     $('.gs_bar').select2();
    ...
    $('.search_bar').autocomplete({
        source: function(request, response) {
            ...
            $.ajax({
                url: 'https://34.204.105.219:8000/product/' + request.term + "/",
                type: 'GET',
                dataType: 'json',
    ...
    $(document).on('click', '.btn-one-way', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                else if(result.type == 'purchase_return'){
                    $("#purchasereturn").trigger('click');
                }
            },
        });
    });
    ...
    var tax_rates_table = $('.tax_rates_table').DataTable({
        ...
        columns: [
            { data: 'name', name: 'name' },
            { data: 'begining_date', name: 'begining_date' },
            ...
            { data: 'every', name: 'every' },

            { data: 'tax_percent', name: 'tax_percent' },
            { data: 'city_tax_value', name: 'city_tax_value' },
            { data: 'first_item_value', name: 'first_item_value' },
            { data: 'second_item_value', name: 'second_item_value' },
            { data: 'everycity', name: 'everycity' },
    ...
    $(document).on('submit', 'form#tax_rate_add_form', function(e) {
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == true) {
                    $(".tax_rates_table").dataTable().fnDraw();
    ...
    $(document).on('ifChanged', '#exact_search_contact,#inactive_contact,#inactive_supplier', function () {
        contact_table.ajax.reload();
    });
    $(document).on('change', '#contact_state,#contact_city,#contact_filter_customer_id,#contact_account_rep,#contact_sales_rep',
        function () {
            contact_table.ajax.reload();
    ...
    if (contact_table_type == 'supplier') {
        var columns = [
            ...
            { data: 'created_username', name: 'createdfor.username' },
            { data: 'tax_number', name: 'tax_number' },
            { data: 'pay_term', name: 'pay_term', searchable: false, orderable: false },
	        { data: 'due', searchable: false },
            { data: 'return_due', searchable: false },
        ...
        var columns = [
            { data: 'check_box', searchable: false, orderable: false},
            { data: 'action', searchable: false, orderable: false },
            { data: 'contact_id', name: 'contact_id' },
            { data: 'supplier_business_name', name: 'supplier_business_name' },
            { data: 'name', name: 'name' },
            ...
            { data: 'pay_term', name: 'pay_term', searchable: false, orderable: false },
            { data: 'due_raw', name:'due_raw', searchable: false, visible:false},
        ...
        Array.prototype.push.apply(columns, [
            ...
            { data: 'account_rep', name: 'account_rep' },
            { data: 'is_nyc', name: 'is_nyc' },
            // { data: 'custom_field1', name: 'custom_field1'},
            // { data: 'custom_field2', name: 'custom_field2'},
            // { data: 'custom_field3', name: 'custom_field3'},
            // { data: 'custom_field4', name: 'custom_field4'},
    ...
    //     //     });
    //     //     console.log('hi')
    //     //     if ($('form#contact_add_form').valid()) {
    //     //         $('form#contact_add_form').submit();
    //     //     }
    ...
    //     $(document).on('click', '.submit_edit_contact_form', function (e) {

    //         e.preventDefault();
    //         var submit_type = $(this).attr('value');
    //         $('#submit_type').val(submit_type);
    //         $('form#contact_edit_form1').validate({
    //             rules: {
    //                 contact_id: {
    //                     remote: {
    //                         url: '/contacts/check-contact-id',
    ...
    //         if ($('form#contact_edit_form1').valid()) {
    //             $('form#contact_edit_form1').submit();
    //         }
    //     });
    ...
    $('.contact_modal').on('shown.bs.modal', function(e) {
       ...
       $('#any_id_expiration_date').datepicker({
        ...
        (function () {
            ...
            $("select#select_address").on('focus', function () {
        ...
        $('form#contact_add_form, form#contact_edit_form1').submit(function(e) {
                e.preventDefault();
            })
            .validate({
                // rules: {
    ...
    $('#type').change(function() {
        show_product_type_form();
        var type = $('#type').val();
        if(type == 'variable') $(".variation-select").addClass('hide');
        else $(".variation-select").removeClass('hide');
    });
    ...
    // added by developer 1 for delete expenses
    $(document).on('click', 'a.delete_expense', function(e) {
        e.preventDefault();
        href = $(this).attr('href');
        var config = {
    ...
    $(document).on('change', '.payment_types_dropdown', function() {
        ...
        if(payment_type == 'advance'){
            ...
            if(parseFloat(advance_balance) < parseFloat(amount)){
                $('.amt_err').html("You can not pay more then "+ advance_balance+" from advance amount.");
                $('#error_count').val(1);
            }
        }
    });
    ...
    $(document).on('click','a.supplierpayment_modal',function(e){
                ...
                $("#saveandnew").click(function(e){
                    ...
                    .validate({
                        submitHandler: function(form) {
                            ...
                            {
                                sells_count = $("#sells_table input[type=checkbox]:checked").length;
                                sells_amount = parseFloat($('#sell_total').val());
                            ...
                            var sells_return_amount = "";
                            var sells_return_count = 1;

                            if($("#sells_return_table").length)
                            {
                                sells_return_count = $("#sells_return_table input[type=checkbox]:checked").length;
                                sells_return_amount = parseFloat($('#sell_return_total').val());
                            }

                            if(sells_return_count==0)
                            {
                                sells_return_amount = 0;
                            }

                            if($("#method").val()=="advance" && !$("#sells_table").length)
                            {
                              toastr.error("Sorry!! There are no invoices to pay with advance amount");
                            }
                            else if($("#method").val()=="credit_memo" && !$("#sells_return_table").length)
                            {
                              toastr.error("Sorry!! There are no credit memo. Please choose another payment method");
                            }
                            else if($("#method").val()=="credit_memo" && !$("#sells_table").length && $("#sells_return_table").length)
                            {
                              toastr.error("Sorry!! There are no invoices to pay against credit memo");
                            }
                            else if(!$("input[name='add_advance']").is(":checked") && sells_amount===0 && parseFloat(add_discount_total)==0 && ($("#method").val()=="advance" || $("#method").val()=="credit_memo"))
                            {
                              toastr.error("Please select and apply due amount.");
                            }
                            else if(!$("input[name='add_advance']").is(":checked") && $("#method").val()=="credit_memo" && sells_return_amount===0)
                            {
                              toastr.error("Please select credit memo and enter due amount.");
                            }
                            else if(!$("input[name='add_advance']").is(":checked") && sells_return_count!=0 && sells_count!=0 && $("#method").val()=="credit_memo" && sells_amount===0 && sells_return_amount>0)
                            {
                              toastr.error("Please enter amount dues along with total Credit memo!!");
                            }
                            else if(!$("input[name='add_advance']").is(":checked") && sells_return_count!=0 && sells_count!=0 && $("#method").val()=="credit_memo" && sells_amount > sells_return_amount && sells_amount>0 && sells_return_amount>0)
                            {
                              toastr.error("Total Credit memo due amount must be greater or equalTo Total due amount!!");
                            }
                            else
                            {
                                var flag_pay = 'false';
                                var data = $(form).serialize();
                                if(!$("input[name='add_advance']").is(":checked") && sells_amount===0 && $('#amount').length)
                                {
                                    swal({
                                        title: "Are you sure to apply automatically?",
                                        icon: 'warning',
                                        buttons: true,
                                        dangerMode: true,
                                    }).then(willPayAutomatically => {
                                        if (willPayAutomatically) {
                                            flag_pay = 'true';
                                            payfromprofile(data,form)
                                        }
                                        else
                                        {
                                            flag_pay = 'false';
    ...
    function payfromprofile(data,form)
    {
        $.ajax({
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == 1 && result.receipt.html_content != '') {
                    ...
                    if (typeof replaceTitleBeforePrint === 'function') {
                        replaceTitleBeforePrint('receipt_section');
                    }
    ...
    $(document).on('click', '.delete-sale', function(e) {
        ...
        href = $(this).attr('href');
        is_suspended = $(this).hasClass('is_suspended');
...
$(document).on('click', '#edit-selected', function (e) {
    ...
    if (selected_rows.length > 0) {
        $('input#selected_customer_for_edit').val(selected_rows);
        $("#customer_id").val(selected_rows);
        $("#bulkeditmodal").modal("show");

    } else {
        $('input#selected_customer').val('');
...
$(document).on('click', 'a.reconcile_payment', function(e) {
