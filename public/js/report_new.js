$(document).ready(function () {
    ...
    var stock_report_cols = [
        ...
        { data: 'total_purchase', name: 'Total Purchase', searchable: false },
        // {data: 'total_sell', name: 'Total Sell', searchable: false},
         {
            data: 'consignment_stock',
            name: 'consignment_stock',
            searchable: true,
            render: function (data, type, row) {
                return `<span class="display_currency" data-currency_symbol="false">${
                    data ?? 0
                }</span>`
            },
        },
        { data: 'total_sold', name: 'Total Sell', searchable: false },
    ]
    ...
    if ($('#stock_date_filter').length == 1) {
        $('#stock_date_filter').daterangepicker(dateRangeSettings, function (start, end) {
            $('#stock_date_filter').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            )
            stock_report_table.ajax.reload()
        })
        $('#stock_date_filter').on('cancel.daterangepicker', function (ev, picker) {
            $('#stock_date_filter').val('')
            stock_report_table.ajax.reload()
        })
    }
    ...
    $(document).on('ifChanged', '#include_nfs', function () {
        stock_report_table.ajax.reload()
    })
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                d.unit_id = $('#unit').val()
                d.include_nfs = $('#include_nfs').is(':checked') ? 1 : 0
                d.supplier_id = $('#supplier_id').val()
                var stock_start = ''
                var stock_end = ''
                if ($('#stock_date_filter').val()) {
                    stock_start = $('input#stock_date_filter')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD')
                    stock_end = $('input#stock_date_filter')
                        .data('daterangepicker')
                        .endDate.format('YYYY-MM-DD')
                }
                d.start_date = stock_start
                d.end_date = stock_end
                // console.log(stock_start, stock_end)
                // if($('#start-date').val()) {
                //     d.start_date = formatDate($('#start-date').val());
                //     d.end_date = formatDate(new Date());
                // }
                d.only_mfg_products =
                    $('#only_mfg_products').length && $('#only_mfg_products').is(':checked') ? 1 : 0
            },
        },
        ...
        fnDrawCallback: function (oSettings) {
            ...
            $('#footer_total_purchased').html(
                __sum_stock($('#stock_report_table'), 'total_purchased')
            )
    ...
    $(
        '#stock_report_filter_form #location_id, #supplier_id, #stock_report_filter_form #category_id, #stock_report_filter_form #sub_category_id, #stock_report_filter_form #brand, #stock_report_filter_form #unit,#stock_report_filter_form #view_stock_filter'
    ).change(function () {
        stock_report_table.ajax.reload()
    ...
    product_sell_report = $('table#product_sell_report_table').DataTable({
        ...
        columns: [
            { data: 'product_name', name: 'p.name' },
            { data: 'sub_sku', name: 'v.sub_sku' },
            { data: 'category', name: 'p.category_id' },
            { data: 'subcategory', name: 'p.sub_category_id' },
            { data: 'customer', name: 'c.name' },
            { data: 'contact_id', name: 'c.contact_id' },
            { data: 'address', name: 'c.address_line_1' },
            { data: 'city', name: 'c.city' },
            { data: 'state', name: 'c.state' },
            { data: 'zip_code', name: 'c.zip_code' },
    ...
    $('#br_filter_date_range').daterangepicker(
        //this is comming from balance report blade file kindly if wants to change Go In Balance report File ;
        datesettings,
        function (start, end) {
            $('#br_filter_date_range').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            )
            balance_report_table.ajax.reload()
        }
    )
    var start = $('#br_filter_date_range').data('daterangepicker').startDate.format('YYYY-MM-DD')
    var end = $('#br_filter_date_range').data('daterangepicker').endDate.format('YYYY-MM-DD')
    //Balance report table
    balance_report_table = $('#balance_report_tbl').DataTable({
        processing: true,
        serverSide: true,
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                if ($('#br_filter_date_range').val()) {
                    var start = $('#br_filter_date_range')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD')
                    var end = $('#br_filter_date_range')
                        .data('daterangepicker')
                        .endDate.format('YYYY-MM-DD')
                    d.start_date = start
                    d.end_date = end
                    if ($('#do_not_show_paid_invoices').is(':checked')) {
                        d.do_not_show_paid_invoices = 1
                    }
                }
            },
        },
        columns: balance_report_cols,
        fnDrawCallback: function (oSettings) {
            $('#footer_total_stock').html(__sum_stock($('#balance_report_tbl'), 'current_stock'))
            $('#footer_total_sold').html(__sum_stock($('#balance_report_tbl'), 'total_sold'))
            $('#footer_total_transfered').html(
                __sum_stock($('#balance_report_tbl'), 'total_transfered')
            )
            $('#footer_total_adjusted').html(
                __sum_stock($('#balance_report_tbl'), 'total_adjusted')
            )
            var total_stock_price = sum_table_col($('#balance_report_tbl'), 'total_stock_price')
            $('#footer_total_stock_price').text(total_stock_price)
            ...
            var total_potential_profit = sum_table_col($('#balance_report_tbl'), 'potential_profit')
            $('#footer_potential_profit').text(total_potential_profit)

            __currency_convert_recursively($('#balance_report_tbl'))
            if ($('th.current_stock_mfg').length) {
                $('#footer_total_mfg_stock').html(
                    __sum_stock($('#balance_report_tbl'), 'total_mfg_stock')
                )
            }
        },
        footerCallback: function (row, data, start, end, display) {
            var footer_total_open_balance = 0
            for (var r in data) {
                footer_total_open_balance += $(data[r].total_remaining).data('orig-value')
                    ? parseFloat($(data[r].total_remaining).data('orig-value'))
                    : 0
            }
            $('.footer_total_open_balance').html(
                __currency_trans_from_en(footer_total_open_balance)
            )
        },
    })
    ...
    $('#do_not_show_paid_invoices').on('ifChanged', function (event) {
        balance_report_table.ajax.reload()
    })
})
