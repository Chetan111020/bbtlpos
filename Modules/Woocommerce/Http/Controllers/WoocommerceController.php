...
class WoocommerceController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $not_synced_brands_count = Brands::where('business_id', $business_id)
                ->whereNull('woocommerce_brand_id')
                ->count();

            if (!empty($not_synced_brands_count)) {
                $alerts['not_synced_brands'] = $not_synced_brands_count == 1 ? "1 Brand not Synced" : $not_synced_brands_count . " Brands not synced";
            }

            $brands_last_sync = $this->woocommerceUtil->getLastSync($business_id, 'brands', false);
            if (!empty($brands_last_sync)) {
                $updated_brands_count = Brands::where('business_id', $business_id)
                    ->whereNotNull('woocommerce_brand_id')
                    ->where('updated_at', '>', $brands_last_sync)
                    ->count();
            }

            if (!empty($updated_brands_count)) {
                $alerts['updated_brands'] = $updated_brands_count == 1 ? "1 Brand has been updated after last sync" : $updated_brands_count . " Brands have been updated after last sync";
            }
            ...
            $query = Product::where('business_id', $business_id)
                ...
                ->select('products.*', 'vld.qty_available as current_stock');

            if (!empty($woocommerce_api_settings->location_id)) {
                $query->ForLocation($woocommerce_api_settings->location_id);
            }
            // $not_synced_product_count = $query->get();

            $not_synced_product_count = Product::where('not_for_selling','<>',1)
            ...
            if (!empty($not_synced_product_count)) {
                $alerts['not_synced_product'] = $not_synced_product_count;
            }
            ...
            if (!empty($products_last_synced)) {
                ...
                //     ->where('products.woocommerce_disable_sync', 0)
                //     ->where('not_for_selling',0)
                //     ->whereIn('products.type', ['single', 'variable'])
                //     ->where('products.updated_at', '>', $products_last_synced)
                //     ->join('variations as v', 'v.product_id', '=', 'products.id')
                //     ->leftJoin('variation_location_details as vld', 'vld.variation_id', '=', 'v.id')
                //     ->select('products.*', 'vld.qty_available as current_stock')
                //     ->get();
                ...
                $updated_product_count = Product::where('not_for_selling','<>',1)
                    ->where('woocommerce_disable_sync','<>',1)
                    ->where('business_id',4)
                    ->whereIn('type', ['single', 'variable'])
            ...
            if (!empty($updated_product_count)) {
                $alerts['not_updated_product'] = $updated_product_count;
            }
            ...
            $alerts['inactive_product_count'] = $inactive_products;
    ...
    /**
     * Synchronizes pos customer with Woocommerce customer
     * @return Response
     */
    public function syncCustomer($id)
    {
        ...
        try {
            ...
            $this->woocommerceUtil->syncCust($business_id, $user_id, $id);
    ...
    public function customerResetPassword($id)
    {
        ...
        try {
            ...
            $this->woocommerceUtil->customerResetPassword($business_id, $user_id, $id);

            DB::commit();

            $output = [
                'success' => 1,
                'msg' => __("Reset Password Successfully.")
            ];
        } catch (\Exception $e) {
    ...
    public function checkWooCustomerExistOrNot($id)
    {
        $business_id = request()->session()->get('business.id');
        ...
        try {
            ...
            $data = $this->woocommerceUtil->checkWooCustomerExistOrNot($business_id, $user_id, $id);
            ...
            if (!empty($data)) {
                $output = [
                    'success' => 1,
                    'msg' => __("Customer Verified Successfully.")
                ];
            } else {
                $output = [
                    'success' => 0,
                    'msg' => __("Customer does not exits.")
                ];
            }
        } catch (\Exception $e) {
    ...
    public function syncBrands()
    {
        ...
        try {
            ...
            $this->woocommerceUtil->syncBrands($business_id, $user_id);
    ...
    public function syncProducts()
    {
        ...
        try {
            ...
            if (isset($ids)) {
                ...
                $all_products = $this->woocommerceUtil->syncProductsWithId($business_id, $user_id, $sync_type, $limit, $offset, $product_ids);
            } else {
    ...
    /**
     * Delete not for selling products from woocommerce
     * @return Response
     */
    public function syncProductsDelete()
    {
        $notAllowed = $this->woocommerceUtil->notAllowedInDemo();
        if (!empty($notAllowed)) {
            return $notAllowed;
        ...
        try {
            ...
            if (isset($ids)) {
                $product_ids = explode('-', $ids);
                // to be implemented
                $all_products = $this->woocommerceUtil->syncProductDelete($business_id, $user_id, $limit, $offset);
            } else {
                $all_products = $this->woocommerceUtil->syncProductDelete($business_id, $user_id, $limit, $offset);
            }
            ...
            $msg = $all_products[0] . " Product(s) Deleted Successfully";
            $output = [
                'success' => 1,
                'msg' => $msg,
                'total_products' => $all_products[0],
                'total_products2' => $all_products[1],
                'page' => $offset
            ];
     ...
     * @return Response
     */
    public function syncProductsSelect()
    {
        $notAllowed = $this->woocommerceUtil->notAllowedInDemo();
        if (!empty($notAllowed)) {
            return $notAllowed;
    ...
    public function syncOrdersSelect()
    {
        $notAllowed = $this->woocommerceUtil->notAllowedInDemo();
        if (!empty($notAllowed)) {
            return $notAllowed;
        ...
        try {
            ...
            $this->woocommerceUtil->syncOrdersWithId($business_id, $user_id, $order_ids);
    ...
    public function getSyncLog()
    {
        ...
        if (request()->ajax()) {
            $last_sync = [
                ...
                'brands' => $this->woocommerceUtil->getLastSync($business_id, 'brands')
            ];
            return $last_sync;
        }
    }
        ...
        if (request()->ajax()) {
            $logs = WoocommerceSyncLog::where('woocommerce_sync_logs.business_id', $business_id)
                ...
                ->select([
                    ...
                    'woocommerce_sync_logs.details as error_details'
                ]);

            $synctype = request()->get('synctype', null);
            if ($synctype == 'error') {
                $logs->where('details', '!=', null);
            } else {
                $logs->where('sync_type', $synctype)
                    ->orWhere('operation_type', $synctype);
            }
        ...
        if (request()->ajax()) {
            try {
                Brands::where('business_id', $business_id)
                    ->update(['woocommerce_brand_id' => null]);
                $user_id = request()->session()->get('user.id');
                $this->woocommerceUtil->createSyncLog($business_id, $user_id, 'brands', 'reset', null);

                $output = [
                    'success' => 1,
                    'msg' => "Brands reset successfully",
                ];
            } catch (\Exception $e) {
                \Log::emergency("File:" . $e->getFile() . "Line:" . $e->getLine() . "Message:" . $e->getMessage());
