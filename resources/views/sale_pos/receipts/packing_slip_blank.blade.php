                            ...
                            <tbody id="mytable" style="display:none;">
                                ...
                                @forelse($receipt_details->lines_sorted_by_name as $line)
                                    <tr>
                                        ...
                                        if(@$line['cat']['name'] == 'Disposables') { 
                                          $pattern = '/(PF|PUFFS)([^-]*)/';
                                          $line['name'] = preg_replace($pattern, '$1', $line['name']);
                                        }
                                        ?>
                                        <td
...
<h4 style="text-align: right;display:none;">{{ $blank_slip_number ?? '' }}</h4>
</div>
</body>
...
//             Subtotal: 0.0
//         };
//         result.push(merged[a.product]);
//     }
//     merged[a.product].Qty += Math.round(a.Qty);
//     merged[a.product].Subtotal += parseFloat(a.Subtotal);
// });
...
//     for (i = 0; i < result.length; i++) {
//         sr = sr + 1;
//         k += (result[i].Qty == 0) ? '<tr class="order-list">' : '<tr>';
//         k += '<td class="hide" style="vertical-align: top;padding-left: 5px;text-align: center; border: 1px solid #808080;">' + sr + '</td>';
//         k += '<td class="hide" style="vertical-align: top;padding-left: 5px;text-align: center; border: 1px solid #808080;">' + result[i].Location + '</td>';
//         k += '<td class="hide" style="vertical-align: top;padding-left: 5px;text-align: left; border: 1px solid #808080;">' + result[i].Barcode + '</td>';
//         k += '<td style="vertical-align: top;padding-left: 5px;text-align: center; border: 1px solid #808080;">' + result[i].Qty + '</td>';
//         k += '<td style="vertical-align: top;padding-left: 5px;text-align: left; border: 1px solid #808080;">' + result[i].product + '</td>';
//         k += '<td class="hide" style="vertical-align: top;padding-left: 5px;text-align: left; border: 1px solid #808080;">' + result[i].item_code + '</td>';
//         k += '<td style="vertical-align: top;padding-left: 5px;text-align: center; border: 1px solid #808080;"> $ ' + result[i].Price + '</td>';
//         k += '<td class="hide" style="vertical-align: top;padding-left: 5px;text-align: center; border: 1px solid #808080;"> $ ' + result[i].tax + '</td>';
//         k += '<td style="vertical-align: top;padding-left: 5px;text-align: center; border: 1px solid #808080;"> $ ' + parseFloat(result[i].Subtotal).toFixed(2) + '</td>';
//         k += '</tr>';
//     }
//     k += `<tr style="display:none;">
//                 <th></th>
...
// for merging duplicates based on both product name and price
var result = [];
var merged = {};

details.forEach(function(a) {
    // Generate a unique key using both product name and price
    var key = a.product + "_" + a.Price;
    ...
    // Accumulate quantities and subtotals for matching products with the same name and price
    merged[key].Qty += Math.round(a.Qty);
    merged[key].Subtotal += parseFloat(a.Subtotal);
});

// for displaying the table with the merged products
function renove() {
    $('#mytable').remove();
