...
<div class="modal-body">
    ...
    <div class="row">
      ...
      <div class="col-sm-4">
        ...
        @endif
        <b>Customer Name:</b> {{ $sell->contact->name }}<br>
        <b>Customer Email:</b> {{ !empty($sell->contact->email) ? $sell->contact->email : "--" }}<br>
              ...
              <tbody>
                @forelse($activity_logs as $log)
                  <tr>
                    ...
                    <td width="10%">{{ Carbon\Carbon::parse($log->datetime)->format('m/d/Y H:i A') }}</td>
                  </tr>
                @empty
                ...
                @foreach($pickpacklog as $log)
                  <tr>
                    ...
                    @endif
                    @if($log->type == 'start picking')
                        <td>#Picking Started by {{$log->first_name}} at {{ Carbon\Carbon::parse($log->datetime)->format('d-m-Y H:i:s') }}</td>
                    @elseif($log->type == 'start packing')
                        <td>#Packing Started by {{$log->first_name}} at {{ Carbon\Carbon::parse($log->datetime)->format('d-m-Y H:i:s') }}</td>
                    @endif
                        <br>
                    {{-- <td width="10%"> {{ Carbon\Carbon::parse($log->datetime)->format('d-m-Y G:i A') }}</td> --}}
                    <td width="10%">{{ Carbon\Carbon::parse($log->datetime)->format('m/d/Y H:i A') }}</td>
