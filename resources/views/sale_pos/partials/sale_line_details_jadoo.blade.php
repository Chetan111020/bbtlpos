<table class="table bg-gray">
    <thead>
        <tr class="bg-green">
            ...
            <th class='text-right'>{{ __('sale.qty') }}</th>
            ...
            <th class='text-right'>{{ __('sale.tax') }}</th>
    ...
    <tbody id="jtbd">
        <tr class="jro" style="display:none;">
            <td>{{ $loop->iteration }}</td>
            <td class="jropn">
                <?php
                    $key="DESIGNER FLORAL DIAMOND PRINT WP -IFBR";
                    $line_name = $sell_line->product->name;
                ?>
                @if($line_name == 'INFINITE BAR DISPOSABLE 5000PF PEACH -BOX OF 10')
                {{$key}}
                @elseif($line_name == 'INFINITE BAR DISPOSABLE 5000PF LYCHEE ICE -BOX OF 10')
                {{$key}}
                @elseif($line_name == 'INFINITE BAR DISPOSABLE 5000PF BLUEBERRY ICE -BOX OF 10')
                {{$key}}
                @elseif($line_name == 'INFINITE BAR DISPOSABLE 5000PF LUSH ICE -BOX OF 10')
                {{$key}}
                @else
                    @if(isset($jadoo_products['product']) && !empty($jadoo_products['product']))
                        ...
                        @foreach($jadoo_products['product'] as $match => $value)
                            @if(strpos(strtolower($line_name),strtolower($match))!== false)
                                @php
                                    $final_line_name = $value;
                                @endphp
                            @elseif(strtolower($line_name) == strtolower($match))
                                @php
                                    $final_line_name = $value;
                                @endphp
                            @endif
            ...
            @if ($sell_line->packing_started_time)
                <td>{{Carbon\Carbon::parse($sell_line->packing_started_time)->format('m-d-Y G:i A')}}</td>
            @else
                <td>N/A</td>
            @endif
            @if ($sell_line->packing_completed_time)
                <td>{{Carbon\Carbon::parse($sell_line->packing_completed_time)->format('m-d-Y G:i A')}}</td>
            @else
                <td>N/A</td>
            @endif
            @if( session()->get('business.enable_lot_number') == 1)
