<table class="table" style="color:black">
    <tr class="" style="background:#e0f2fe">
        ...
        <th style="width: 55%;">{{ __('sale.product') }}</th>
        @if( session()->get('business.enable_lot_number') == 1)
            <th>{{ __('lang_v1.lot_n_expiry') }}</th>
        @endif
        <th style="width: 100px;">Picking Started At</th>
        <th style="width: 120px;">Packing Completed At</th>
        <th>{{ __('sale.qty') }}</th>
    ...
    @foreach($sell->sell_lines as $sell_line)
        <tr>
            ...
            <td>
                @php
                    if(!isset($sell_line->product)){
                        $sell_line->product = $deleted_product;
                    }
                @endphp
            ...
            @if ($sell_line->packing_started_time)
                <td style="font-size: smaller;">{{$sell_line->packing_started_time}}</td>
            @else
                <td>N/A</td>
            @endif
            @if ($sell_line->packing_completed_time)
                <td style="font-size: smaller;">{{$sell_line->packing_completed_time}}</td>
            @else
                <td>N/A</td>
            @endif
            @if( session()->get('business.enable_lot_number') == 1)
            ...
            @endif
            <td>
                <span data-currency_symbol="false" data-is_quantity="true">{{ round($sell_line->quantity) }}</span>
