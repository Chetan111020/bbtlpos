<div class="row">
	...
	<div class="{{$col_class}}">
		<div class="form-group">
			...
			<div class="input-group">
				...
				@php
				$payamount = (isset($paymentamount) && $paymentamount) ? '' : 'payment-amount';
				@endphp
				{!! Form::text("payment[$row_index][amount]", @num_format($payment_line['amount']), ['class' => "form-control $payamount input_number", 'required', 'id' => "amount_$row_index", 'placeholder' => __('sale.amount'), 'readonly' => $readonly]); !!}
			</div>
		</div>
	</div>
	@if(!empty($show_date))
	<div class="{{$col_class}}">
		<div class="form-group">
			...
			<div class="input-group">
              ...
              {!! Form::text("payment[$row_index][paid_on]", isset($payment_line['paid_on']) ? @format_datetime($payment_line['paid_on']) : @format_datetime('now'), ['class' => 'form-control paid_on', 'required']); !!}
            </div>
		</div>
	</div>
	@endif
    ...
    {{-- <div class="{{$col_class}}">
		<div class="form-group">
			<label id="tr_no_label" for="transaction_no">Transaction No:</label>
			<div class="input-group">
              <span class="input-group-addon">
                <i class="fa fa-hashtag"></i>
              </span>
              {!! Form::text("payment[$row_index][transaction_no]",$payment_line->transaction_no ?? '', ['class' => 'form-control', 'id' => 'transaction_no', 'placeholder' => __('lang_v1.transaction_no')]); !!}
            </div>
		</div>
	</div> --}}
