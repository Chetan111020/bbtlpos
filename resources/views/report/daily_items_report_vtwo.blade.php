...
@section('content')
    ...
    <!-- /.content -->
    <section class="content-header">
        <h1>Back-In-Stock Items </h1> 
    </section>
    <!-- Main content -->
    <section class="content">
        <p><span style="color:orange;">(Takes Time to Process)</span><p>
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="back_in_stock_items">
                           <thead>
                        <tr>
                            <th>Id</th>
                            <th>Product Name</th>
                            ...
                            <th>Product SKU</th>
                            <th>Purchase Qty</th>
                    <!--    <th>On-Hand Qty(EOD)</th> -->
                            <th>Pre-Purchase Qty</th>
                            <th>Selling Price</th>
                            <th>Category</th>
                            <th>Sub Category</th>
                        </tr>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $('#reportrange').datepicker().change(function() {
          ...
          //{
                ...
                if(selected_date!="")
                {
                    ...
                    back_in_stock_item_report='';
                    getBackInStockItems(selected_date);
                }
            //count = 0;
          //}
        });
        ...
        function getBackInStockItems(selected_date) {
    back_in_stock_item_report = $('#back_in_stock_items').DataTable({
        destroy: true,
        processing: true,
        serverSide: false,
        ajax: {
            url: '{{ url("reports/purchased-items-list-vthr") }}',
            type: "GET",
            data: function (dd) {
                dd.select_date = selected_date;
            }
        ...
        columns: [
            ...
            {data: 'sku', name: 'v.sub_sku'},
            {data: 'purchase_qty', name: 'purchase_lines.quantity'},
           // {data: 'on_hand_qty', name: 'on_hand_qty'},
            {data: 'ofs_qty', name: 'ofs_qty',render: function(data, type, row) {return parseInt(data); }},
            {data: 'selling_price', searchable: false},
            {data: 'category', name: 'c1.name'},
            {data: 'sub_category', name: 'c2.name'},
        ],
        createdRow: function (row, data, dataIndex) {
            // Check if On-Hand Qty(EOY) is greater than 0
            if (data.ofs_qty > 0) {
                $(row).hide(); // Hide the row if On-Hand Qty(EOY) is positive
            }
        },
        fnDrawCallback: function (oSettings) {
            __currency_convert_recursively($('#back_in_stock_items'));
        },
        columnDefs: [
            {
                targets: 4,
