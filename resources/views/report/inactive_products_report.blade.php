...
<section class="content-header">
    <h1>{{ __('Inactive Items Report')}}</h1>
</section>
            ...
            @component('components.filters', ['title' => __('report.filters')])
              {!! Form::open(['url' => action('ReportController@getStockReport'), 'method' => 'get', 'id' => 'stock_report_filter_form' ]) !!}
                ...
                <div class="col-md-3">
                    ...
                    <div class="form-group">
                        {!! Form::label('inactive_items_filter', __('report.date_range') . ':') !!}
                        {!! Form::text('inactive_items_filter', @format_date('first day of this month') . ' ~ ' . @format_date('last day of this month'), ['placeholder' => __('lang_v1.select_a_date_range'), 'class' => 'form-control', 'id' => 'inactive_items_filter', 'readonly']); !!}
...
<script type="text/javascript">
	$(document).ready( function() {
        ...
        $('#inactive_items_filter').daterangepicker({
                ranges: ranges,
            ...
            $('#inactive_items_filter').on('apply.daterangepicker', function(ev, picker) {
            ...
            $('#inactive_items_filter').on('cancel.daterangepicker', function(ev, picker) {
                $(this).val('');
                inactive_item_report.ajax.reload();
        ...
        inactive_item_report = $('table#inactive_item_report').DataTable({
            ...
            ajax: {
                url: '/reports/inactive-items-report',
                data: function(d) {
                    ...
                    d.unit_id = $('#unit').val();
                    if($('#inactive_items_filter').val()) {
                        var start = $('input#inactive_items_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
                        console.log(start);
                        var end = $('input#inactive_items_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
                        d.start_date = start;
                        d.end_date = end;
                    }
                },
