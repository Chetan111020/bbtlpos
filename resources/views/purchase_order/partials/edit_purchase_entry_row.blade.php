...
@endphp
   ...
   {{-- {{$purchase_line}} --}}
        <tr>
            ...
            <td>
                <span class="case_qty">{{ !empty($purchase_line->product->case_qty) ? $purchase_line->product->case_qty : 0 }}</span>
            </td>
            <td>
                ...
                {!! Form::hidden('purchases[' . $loop->index . '][purchase_line_id]',$purchase_line->id); !!}
                    ...
                    @php
                        ...
                        $quantity_precision = config('constants.quantity_precision', 2);
                    @endphp
                    {!! Form::text('purchases[' . $loop->index . '][quantity]', number_format($purchase_line->quantity, $quantity_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['data-row' => $row_count,  'class' => 'form-control input-sm purchase_quantity input_number mousetrap', 'required', 'data-rule-abs_digit' => $check_decimal, 'data-msg-abs_digit' => __('lang_v1.decimal_value_not_allowed')]); !!}
    ...
    @endforeach
     ...
     @foreach($temp_products as $temp_product)
         <tr>
                <td><span class="sr_number"></span></td>
                <td>
                 <input type="hidden" id="tax_amount" value="0" name="tax_amount">
                 <input type="hidden" id="quantity" value="0" name="quantity">
                 <input type="hidden" id="ProductTemp[{{$loop->index}}][pp_without_discount]" value="{{$temp_product->id}}" name="ProductTemp[{{$loop->index}}][product_id]">
                 <input type="hidden" name="ProductTemp[{{$loop->index}}][name]" value="{{ $temp_product->name }}">
                    <div class="text-wrap width-90">
                        {{ $temp_product->name }}
                    </div>
                </td>
                 <td><input type="hidden" name="ProductTemp[{{$loop->index}}][on_hand]">0</td>
                <td><input class="form-control input-sm purchase_quantity input_number mousetrap" required="" data-rule-abs_digit="true" data-msg-abs_digit="Decimal value not allowed" id="input_{{$loop->index}}_box{{$loop->index}}" name="ProductTemp[{{$loop->index}}][quantity]" type="text" value="{{ $temp_product->purchase_qty }}"></td>
                <td><input class="form-control input-sm purchase_unit_cost_without_discount input_number" required="" data-rule-abs_digit="true" id="input_{{$loop->index}}_box{{$loop->index}}" name="ProductTemp[{{$loop->index}}][pp_without_discount]" type="text" value="{{ $temp_product->previous_cost }}"></td>
                <td><input type="hidden" name="ProductTemp[{{$loop->index}}][previous_cost]">{{ $temp_product->previous_cost }}</td>
                <td><input type="hidden" name="ProductTemp[{{$loop->index}}][total]"><span class="row_subtotal_before_tax">{{ number_format($temp_product->purchase_qty * $temp_product->previous_cost, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator) }}</span></td>
                <td><input type="hidden" name="ProductTemp[{{$loop->index}}][total_sold]"><span  class="display_currency sold"></span></td>
                <td><i class="fa fa-times remove_purchase_entry_row text-danger" title="Remove" style="cursor:pointer;"></i></td>
        </tr>
         @endforeach
