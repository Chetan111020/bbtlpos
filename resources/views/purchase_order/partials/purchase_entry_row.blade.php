@foreach( $variations as $variation)
    <tr>
        @if($edit == 0)
        <td><input type="checkbox" class="bulk_hook"/></td>
        @endif
        ...
        <td>
            <span class="case_qty">{{ !empty($product->case_qty) ? $product->case_qty : 0 }}</span>
        </td>
        @if($edit == 0)
        <td>
            <input type="text" class="form-control input-sm" readonly value="{{ $safeStockQty ?? 0 }}" />
        </td>
        <td>
            <input type="text" class="form-control input-sm" readonly value="{{ $bufferStockQty ?? 0 }}" />
        </td>
        @endif
        <td>
            ...
            @php
                ...
                $over_stocked = 0;
                if($edit == 0){
                    $new_qty = (($safeStockQty + $bufferStockQty)-$variation->variation_location_details->first()->qty_available) ?? 1;
                }
                if($new_qty < 0){
                    $over_stocked = $new_qty * -1;
                    $new_qty = 1;
                }
            @endphp
            {!! Form::text('purchases[' . $row_count . '][quantity]', number_format($new_qty, $quantity_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['data-row' => $row_count, 'data-input' => 1, 'class' => 'form-control input-sm purchase_quantity input_number mousetrap', 'required', 'data-rule-abs_digit' => $check_decimal, 'data-msg-abs_digit' => __('lang_v1.decimal_value_not_allowed'), 'id' => 'input_'.$row_count.'_box1']); !!}
            ...
            @if($over_stocked > 0)
            <span class="text-danger bg-white"><b>This item is already <br/> over stocked.</b></span>
            @endif
        </td>
