...
<section class="content no-print">
    ...
    @component('components.widget', ['class' => 'box-primary', 'title' => __('purchase.all_purchases')])
        @can('purchase.create')
            ...
            @endslot
        @endcan
        <form action="{{ route('po.bulk.delete') }}" method="POST" id="bulkdeleteform">
            @csrf
            @include('purchase_order.partials.purchase_table')
            <button type="button" class="btn btn-xs btn-danger"id="virtualsubmit">Delete Selected</button>
        </form>
    @endcomponent
    ...
    <div class="modal fade payment_modal pay_due_modal" tabindex="-1" role="dialog" 
        aria-labelledby="gridSystemModalLabel">
    </div>
    ...
    @include('purchase_order.partials.update_purchase_status_modal')
...
<script>
    ...
    dateRangeSettings.startDate = moment().add(-6, 'day');
    dateRangeSettings.endDate = new Date();
    console.log(dateRangeSettings);
