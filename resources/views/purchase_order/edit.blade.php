...
button.form-control.btn.btn-primary.pull-left.btn-flat {
  ...
  <section class="content-header">
    <h1>@lang('Edit Purchase Order') <i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="@include('purchase_order.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title="" title=""></i></h1>
  </section>
  ...
  <section class="content">
    ...
    <input name="_method" type="hidden" value="PUT">
      ...
      <div class="row">
        ...
        <div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
          <div class="form-group">
            ...
            {!! Form::text('ref_no', $purchase->ref_no, ['class' => 'form-control', 'required', 'readonly' => true]); !!}
            <label id="ref_no-error" class="error" for="ref_no" hidden="">Reference number already used.</label>
          </div>
        </div>
        <div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
          <div class="form-group">
        ...
        {{-- <div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
          <div class="form-group">
            {!! Form::label('received_date', __('Received Date') . ':*') !!}
            <div class="input-group">
              <span class="input-group-addon">
                <i class="fa fa-calendar"></i>
              </span>
              {!! Form::text('received_date', @format_datetime(!$purchase->received_date ? $purchase->transaction_date : $purchase->received_date), ['class' => 'form-control', 'required']); !!}
            </div>
          </div>
        </div> --}}
        <div class="col-sm-3 @if(!empty($default_purchase_status)) hide @endif">
          <div  class="form-group">
            ...
            {!! Form::select('status', $orderStatuses,$purchase->status ? $purchase->status : 'received', ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']); !!}
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
              {!! Form::label('documents', __('purchase.attach_document') . ':') !!} 
              {!! Form::file('documents[]', ['id' => 'upload_document', 'accept' => implode(',', array_keys(config('constants.document_upload_mimes_types'))), 'class' => 'file']); !!}<br>
                  ...
                  <tr >
                    <td class="table-text">
                      <div id="first_document">file-0
                        <a href="{{$purchase->document_path}}" target="_blank" class="btn btn-primary btn-xs" title="View Document"><i class="fa fa-eye" aria-hidden="true"></i> <span class="hidden-xs">View</span>
                        </a> 
                        <a title="Clear selected file" class="deleteDocFirst btn btn-danger btn-xs fileinput-remove fileinput-remove-button" data-id="{{ $purchase->id }}" data-doc="{{$purchase->document}}" data-key="document">
                            ...
                            <tr>
                                ...
                                <td class="table-text">
                                    <div  id="file-{{ $loop->iteration }}">file-{{ $loop->iteration }} 
                                      <a href="{{ $path }}" target="_blank" class="btn btn-primary btn-xs" title="View Document">
                                          <i class="fa fa-eye" aria-hidden="true"></i> <span class="hidden-xs">View</span>
                                      </a> 
                                      <a title="Clear selected files" class="deleteDoc btn btn-danger btn-xs fileinput-remove fileinput-remove-button" data-id="{{ $purchase->id }}" data-doc="{{ $doc_extra }}" data-key="{{ $loop->iteration }}">
                                        <i class="glyphicon glyphicon-trash"></i>  <span class="hidden-xs">Remove</span>
                                      </a>
                                    </div><br>
                                </td>
      ...
      <div class="col-sm-3">
          <div class="form-group">
            ...
            {!! Form::button('Add All Products', array('class' => 'form-control btn btn-primary pull-left btn-flat add_all_products', 'name' => 'add_product', 'disabled' => false)) !!}
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
            {!! Form::label('add_product_low_stock', __('Add Low Stock (100 Products)') . ':') !!}
            {!! Form::button('Add Low Stocks Items', array('class' => 'form-control btn btn-primary pull-left btn-flat add_product_low_stock', 'data-low-stock' => 1, 'name' => 'add_product_low_stock', 'disabled' => false)) !!}
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
            {!! Form::label('add_purchase_products', __('Add Purchase Products') . ':') !!}
            {!! Form::button('Add Purchase Product', array('class' => 'form-control btn btn-primary pull-left btn-flat add_purchase_products', 'data-purchase-product' => 1,'name' => 'add_purchase_products', 'disabled' => false)) !!}
          </div>
        </div>
        ...
        <div class="col-sm-3" style="display: none;">
          <div class="form-group">
            ...
            {!! Form::file('document_two', ['id' => 'upload_document', 'accept' => implode(',', array_keys(config('constants.document_upload_mimes_types')))]); !!}
            <p class="help-block">
          ...
          <div class="col-sm-8 col-sm-offset-2">
            <div class="form-group">
              <div class="input-group">
                ...
                {!! Form::text('search_product', null, ['class' => 'form-control mousetrap', 'id' => 'search_product_two', 'placeholder' => __('lang_v1.search_product_placeholder'), 'autofocus', 'data-edit'=>"1"]); !!}
              </div>
            </div>
    ...
    <div class="row">
      <div class="col-sm-12">
          @include('purchase_order.partials.edit_purchase_entry_row')
        <hr/>
        <div class="col-lg-3">
          <div class="form-group">
                <div class="box-body">
                    <div class="row">
                      <div class="col-xs-6">
                        ...
                        {!! Form::number('additional_charges', $purchase->shipping_charges, ['class' => 'form-control', 'id' => 'additional_charges']); !!}
                        {!! Form::hidden('shipping_charges',$purchase->shipping_charges, ['class' => 'form-control input_number', 'id' => 'shipping_charges']); !!}
                        ...
                        {!! Form::number('discount_charges', $purchase->discount_amount, ['class' => 'form-control', 'id' => 'discount_charges']); !!}
                        {!! Form::hidden('discount_amount',  $purchase->discount_amount, ['class' => 'form-control input_number', 'id' => 'discount_amount']); !!}
          ...
          <table class="pull-right col-md-12">
            <tr>
                ...
                <td class="col-md-5 text-left">
                  @php
                    $total_items = count($purchase->po_lines);
                    if($total_items > 0){
                        $qty_total = 0;
                         foreach($purchase->po_lines as $pl){
                            $qty_total += $pl->quantity;
                        }
                        foreach($temp_products as $pl1){
                            $qty_total += $pl1->purchase_qty;
                        }
                    }
                  @endphp
                  <span id="total_quantity" class="display_currency" data-currency_symbol="false">{{@$qty_total}}</span>
                </td>
            </tr>
            <tr class="hide">
            ...
            <tr class="">
              ...
              <th class="col-md-5 text-left">
                <span id="sh-charges" class="" >$ {{$purchase->shipping_charges}}</span>
              </th>
            </tr>
            <tr class="" id="discount_show">
              ...
              <th class="col-md-5 text-left">
                ...
                <input type="hidden" id="total_discount_input" value="{{$purchase->discount_amount}}"  name="total_discount_name">
              </th>
            </tr>
            <tr>
              <th class="col-md-7 text-right">@lang( 'purchase.net_total_amount' ):</th>
              <td class="col-md-5 text-left">
                <span id="total_subtotal" class="" style="color: #777;">$ {{$purchase->final_total}}</span>
                <!-- This is total before purchase tax-->
            ...
            <td colspan="4">
              ...
              <div class="col-md-3 mt-10">
                <div class="form-group">
                  ...
                  {!! Form::date('shipping_date', $purchase->shipping_date, ['class' => 'form-control']); !!}
                </div>
              </div>
              ...
              <div class="col-md-3 mt-10">
                <div class="form-group">
                  ...
                  {!! Form::text('shipping_carier', $purchase->shipping_carier, ['class' => 'form-control']); !!}
                </div>
              </div>
              ...
              <div class="col-md-3 mt-10">
                <div class="form-group">
                  ...
                  {!! Form::text('tracking_id', $purchase->tracking_id, ['class' => 'form-control']); !!}
                </div>
              </div>
              ...
              {{-- <div class="col-md-3 mt-10">
                <div class="form-group">
                  ...
                  {!! Form::number('shipping_charges', $purchase->shipping_charges, ['class' => 'form-control']); !!}
                </div>
              </div> --}}
              ...
              <div class="col-md-3 mt-10">
                <div class="form-group">
                  ...
                  {!! Form::date('eta', $purchase->eta, ['class' => 'form-control']); !!}
                </div>
              </div>
      ...
      <div class="box-body payment_row">
        <div class="row">
          <div class="col-sm-12">
            ...
            <button class="btn btn-warning pull-right btn-flat">
              <a href="javascript:void(0)" class="convert_to_vendor_bill" style="color: white;">Convert To Vendor Bill</a>
            </button>
            @endif
            <button type="button" id="submit_purchase_form" class="btn btn-primary pull-right btn-flat">Update</button>
            <button class="btn btn-danger pull-right btn-flat"><a href="{{ route('purchases.index') }}" style="color: white;">Cancel</a></button>
          </div>
        </div>
      </div>
    @endcomponent
...
@section('javascript')
  ...
  <script src="{{ asset('js/purchase_order.js?v=' . $asset_v) }}"></script>
  ...
  <script type="text/javascript">
    ...
    $(".deleteDoc").click(function(){
        ...
        $.ajax({
            ...
            dataType: 'json',
            data:{id:id, doc:doc, key:key, _token:token},
