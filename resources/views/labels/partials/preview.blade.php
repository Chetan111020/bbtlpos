			          ...
			          @if(auth()->user()->id == 6)
                            ...
                            @if(!empty($print['reg_and_sales']))
                                @if(!empty($page_product->web_sale_price) && !empty($page_product->srp))
                                 <span style="font-weight: {{$boldFont ? 'bold' : 'normal'}}; margin-bottom: 0px !important; font-size: {{7 * $factor}}px; text-decoration: line-through;">
                                        {{ session('currency')['symbol'] ?? '' }}
                                        {{ @num_format($page_product->srp) }}
                                    </span><br>
                                    <span style="font-weight: {{$boldFont ? 'bold' : 'normal'}}; margin-bottom: 0px !important; font-size: {{7 * $factor}}px">
                                        {{ session('currency')['symbol'] ?? '' }}
                                        {{ @num_format($page_product->web_sale_price) }}
                                    </span><br>
                                @endif
                            @endif
                        @endif
