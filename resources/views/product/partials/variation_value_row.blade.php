...
<tr>
    ...
    <td>
        {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][barcode]', null, ['class' => 'form-control input-sm', 'required']); !!}
    </td>
    <td>
    ...
    <td class="{{$class}}">
        ...
        <div class="input-group">
              {!! Form::hidden($name . '[' . $variation_index . '][variations][' . $value_index . '][dpp_inc_tax]', $default, ['class' => 'form-control input-sm variable_dpp_inc_tax input_number', 'placeholder' => __('product.inc_of_tax'), 'required']);!!}

            {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][default_purchase_price]', $default, ['class' => 'form-control input-sm variable_dpp input_number', 'placeholder' => __('product.inc_of_tax'), 'required']); !!}
            @if($value_index == 0)
                <span class="input-group-btn">
                        <button type="button" class="btn btn-default bg-white btn-flat apply-all btn-sm p-5-5"
                                ...
                                data-target-class=".variable_dpp_inc_tax"><i class="fas fa-check-double"></i></button>
                    </span>
            @endif
        </div>
    </td>
    <td class="{{$class}}">
        {{--<div class="input-group">--}}
            ...
            {{--@endif--}}
        {{--</div>--}}
             {!! Form::hidden($name . '[' . $variation_index . '][variations][' . $value_index . '][sell_price_inc_tax]', $default, ['class' => 'form-control input-sm variable_dsp_inc_tax input_number', 'placeholder' => __('product.inc_of_tax'), 'required']); !!}
    </td>
    <td class="{{$class}}">
        <div class="input-group">
            ...
            @if($value_index == 0)
                <span class="input-group-btn">
                    <button type="button" class="btn btn-default bg-white btn-flat apply-all btn-sm p-5-5"
                            data-toggle="tooltip" title="@lang('lang_v1.apply_all')"
                            data-target-class=".variable_profit_percent"><i class="fas fa-check-double"></i></button>
                </span>
            @endif
        </div>
    </td>
    <td>
        <div class="input-group">
            {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][on_hand]', null, ['class' => 'form-control input-sm variable_stock']); !!}
            @if($value_index == 0)
                <span class="input-group-btn">
                    <button type="button" class="btn btn-default bg-white btn-flat apply-all btn-sm p-5-5"
                            data-toggle="tooltip" title="@lang('lang_v1.apply_all')"
                            data-target-class=".variable_stock"><i class="fas fa-check-double"></i></button>
                </span>
            @endif
        </div>
    </td>
    <td>
        <div class="input-group row">
            <div class="col-3 zero-padding-right">
                {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][aisle]', null, ['class' => 'form-control input-sm variable_location', 'placeholder'=>'A']); !!}
            </div>
            <div class="col-3 zero-padding-left zero-padding-right">
                {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][rack]', null, ['class' => 'form-control input-sm variable_location', 'placeholder'=>'R']); !!}
            </div>
            <div class="col-3 zero-padding-left zero-padding-right">
                {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][shelf]', null, ['class' => 'form-control input-sm variable_location', 'placeholder'=>'S']); !!}
            </div>
            <div class="col-3 zero-padding-left">
                {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][bin]', null, ['class' => 'form-control input-sm variable_location', 'placeholder'=>'B']); !!}
            </div>
        </div>
    </td>
