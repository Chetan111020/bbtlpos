...
@endphp
   ...
   {{-- {{$purchase_line}} --}}
        <tr>
           ...
           <td> <div class="text-wrap width-90">
                    <span class="text-link text-info cursor-pointer product-name" data-toggle="modal"
                            data-target="#vendor_bill_row_modal" data-variation_id="{{$purchase_line->variation_id}}"  data-product_id='{{$purchase_line->product_id}}'>
                    {{ $purchase_line->product->name }} <br /> ({{$purchase_line->variations->sub_sku}})<br />  (Item Code : {{ $purchase_line->product->item_code}})
                    </span>
                    ...
                    @if($purchase_line->product->enable_stock == 1)
                        <br>
                        <small style="white-space: nowrap;">@lang('report.current_stock'): @if(!empty($purchase_line->variations->variation_location_details->first())) {{@num_format($purchase_line->variations->variation_location_details->first()->qty_available)}} @else 0 @endif {{  $purchase_line->product->unit->short_name }}</small> <br>
                        <small style="white-space: nowrap;" class="{{ $row_count }}_show_Hand hide"><span>New Qty On Hand :  <span class="row_on_hand"></span></span></small>
                        <input type="hidden" class="row_product_on_hand" id="{{ $row_count }}_product_on_hand" value=@if(!empty($purchase_line->variations->variation_location_details->first())) {{@num_format($purchase_line->variations->variation_location_details->first()->qty_available)}} @else 0 @endif>
                    @endif
            ...
            <td>
                ...
                <span>Current Cost: $<span class="currentcost">{{ number_format($purchase_line->pp_without_discount/$purchase->exchange_rate, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator)}}</span></span>
                <br/>
                <span> <input type="checkbox" name="purchases[{{$loop->index}}][update_cost]" > Update Cost {{ $purchase_line->update_cost }}</span>
                <br>
            </td>
            <td>
