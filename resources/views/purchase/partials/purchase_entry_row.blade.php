@foreach( $variations as $variation)
    <tr>
        ...
        <td>
            ...
            <div class="text-wrap width-90">
                ...
                <span class="text-link text-info cursor-pointer product-name" data-toggle="modal"
                    data-target="#vendor_bill_row_modal" data-variation_id="{{$variation->id}}"  data-product_id='{{$product->id}}'>
                    {{ $product->name }}   <br>({{$variation->sub_sku}}) <br />(Item Code : {{ $product->item_code }})
                </span>
                ...
                @if($product->enable_stock == 1)
                    <br>
                    <small style="white-space: nowrap;">@lang('report.current_stock'): @if(!empty($variation->variation_location_details->first())) {{@num_format($variation->variation_location_details->first()->qty_available)}} @else 0 @endif {{ $product->unit->short_name }}</small> <br>
            ...
            @if ($product->not_for_selling == 1)
                <span><input type="checkbox" name="notforselling" id="notforselling" checked  >Activate</span>
            @endif

            @if ($product->out_of_stock == 1)
                <span><input type="checkbox" name="outofstock" id="outofstock" checked  >Mark as InStock</span>
            @endif
        </td>
        <td>
        ...
        <td>
            ...
            number_format($variation->dpp_inc_tax, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['data-row' => $row_count, 'data-input' => 3, 'class' => 'form-control input-sm purchase_unit_cost_without_discount input_number', 'required', 'id' => 'input_'.$row_count.'_box3']); !!}
            <br/>
            ...
            <br/>
            <span> <input type="checkbox" name="purchases[{{ $row_count }}][update_cost]"> Update Cost </span>
            <br/>
        </td>
        <td>
            {!! Form::text('purchases[' . $row_count . '][discount_percent]', 0, ['class' => 'form-control input-sm inline_discounts input_number', 'disabled']); !!}
        </td>
        <td>
