<tr class=" @if(!empty($parent_payment_ref_no)) child{{$payment->parent_id}} hide @endif">
    @if(!empty($parent_payment_ref_no))<td></td>@endif
    @if(empty($payment->parent_id))
        <td @if($count_child_payments > 0)  style="vertical-align:middle;" @endif >
            {{@format_datetime($payment->paid_on)}}
        </td>
    @endif
    <td @if(empty($parent_payment_ref_no)) class="bg-gray" @endif>
        {{$payment->payment_ref_no}}
        @if(!empty($parent_payment_ref_no))
            <br>@lang('lang_v1.parent_payment'): {{$parent_payment_ref_no}}
        @endif
    </td>
    <td @if($count_child_payments > 0 || empty($parent_payment_ref_no)) class="bg-gray" @endif>
        <span class="display_currency paid-amount" data-orig-value=" {{$payment->amount}}" data-currency_symbol ="true">${{round($payment->amount, 2)}} <br> @if($sum_child_payments > 0 && $count_child_payments > 0) (Total child payments: ${{round($sum_child_payments, 2)}}) @endif</span>
    </td>
    <td @if($count_child_payments > 0 || empty($parent_payment_ref_no)) class="bg-gray" @endif>
        @php
            ...
            if ($payment->method == 'cheque') {
                ...
                $method .= '<br>(' . __('lang_v1.transaction_no') . ': ' . $payment->transaction_no . ')';
            }elseif ($payment->method == 'custom_pay_6') {
                $method .= '<br>(' . __('Bank Name') . ': ' . $payment->transaction_no . ')';
            }elseif ($payment->method == 'custom_pay_7') {
                $method .= '<br>(' . __('Bank Name') . ': ' . $payment->transaction_no . ')';
            }elseif ($payment->method == 'custom_pay_8') {
                $method .= '<br>(' . __('lang_v1.transaction_no') . ': ' . $payment->transaction_no . ')';
            }elseif ($payment->method == 'custom_pay_9') {
                $method .= '<br>(' . __('lang_v1.transaction_no') . ': ' . $payment->transaction_no . ')';
            }elseif ($payment->method == 'cash' && isset($payment->cash_note)) {
                $method .= '<br>(' . __('Cash Amount') . ': ' . $payment->cash_note . ')';
            }
            
            if ($payment->is_return == 1 && $payment->is_advance == 1 && $payment->advance_amt > 0) { 	
                $method .= '<br><small>(Advance)</small>';	
            }	
            else if ($payment->is_return == 1) {
    ...
    <td @if(empty($parent_payment_created_by)) class="bg-gray" @endif>
            @if(!empty($parent_payment_created_by))
                {{$parent_payment_created_by}}
            @else
                {{$payment->created_by}}
            @endif
    </td>
    <td class="bg-gray">
        {{$payment->payment_location}}
    </td>
    <td @if($count_child_payments > 0 || empty($parent_payment_ref_no)) class="bg-gray" @endif>
        @if($count_child_payments > 0) 
        <button type="button" class="btn btn-primary btn-xs" onClick="toggleChildContent('child{{$payment->id}}')"><i class="fas fa-angle-double-down"></i></button>
        @endif
