...
@section('content')
    ...
    <section class="content no-print">
        <div class="row no-print">
            ...
            <div class="col-md-4 col-xs-12 mt-15 pull-right" id="select-container">
                    ...
                    <div class="box-body">
                        ...
                        <div class="row">
                            <div style="border-color: #00acd6;" class="col-md-4 border-right">
                                ...
                                <strong><i class="fas fa-user-tie"></i> @lang('Payment Status')</strong>
                                <p class="text-muted">
                                    @if( $contact->payment_status  ==  'ask_for_payment_before_ship')
                                        Ask For Payment Before Shipping
                                    @elseif($contact->payment_status  == 'ok_to_ship')
                            ...
                            <div style="border-color: #00acd6;" class="col-md-4 border-left">
                                ...
                                <strong><i class="fa fa-key margin-r-5"></i> {{ __('Woocommerce Username') }}</strong>
                                <p class="text-muted">
                                    {{ !empty($contact->woocommerce_username) ? $contact->woocommerce_username : $contact->contact_id }}
                                </p>
                                <strong><i class="fa fa-key margin-r-5"></i> {{ __('Woocommerce Password') }}</strong>
                                <p class="text-muted">
                                    {{ !empty($contact->woocommerce_password) ? $contact->woocommerce_password : $contact->contact_id.'$Esd@123' }}
                                </p>
                                @endif
                        ...
                        @endif
                            ...
                            @if(isset($roles))
                                @if($roles == 'Admin' || $roles == 'Administration')
                                    <a href="{{ action('TransactionPaymentController@getReconcilePayments', [$contact->id]) }}" style="background-color: #357ca5;float: right;color:#fff;margin-right: 5px;" class="btn btn-flat small-box-footer mark_as_cooked_btn col-md-2 reconcile_payment"><i class="fas fa-money-bill-alt"></i> Reconcile Payments</a>
                                @endif
                            @endif
                        @elseif($contact->type == 'supplier')
                            ...
                            @if(isset($roles))
                                @if($roles == 'Admin' || $roles == 'Administration')
                                    <a href="{{ action('TransactionPaymentController@getReconcilePayments', [$contact->id]) }}?type=purchase" style="background-color: #357ca5;float: right;color:#fff;margin-right: 5px;" class="btn btn-flat small-box-footer mark_as_cooked_btn col-md-2 reconcile_payment"><i class="fas fa-money-bill-alt"></i> Reconcile Payments</a>
                                @endif
                            @endif
                        @endif
...
@endif">
                            ...
                            <li class="
                            @if(!empty($view_type) &&  $view_type == 'purchase_return')
                                    active
@else
                                    ''
@endif">
                                <a href="#purchase_return_tab" data-toggle="tab" id="purchasereturn" aria-expanded="true"><i
                                            class="fas fa-arrow-circle-down"
                                            aria-hidden="true"></i> Vendor Retrun</a>
                            </li>
                            <li class="
...
@endif">
                        ...
                        <li class="
                            @if(!empty($view_type) &&  $view_type == 'followup')
                                active
                            @else
                                ''
                            @endif">
                            <a href="#followups_tab" data-toggle="tab" id="followup_tab" aria-expanded="true"><i
                                class="fa fa-address-card" aria-hidden="true"></i> Follow-Up
                        </a>
                        </li>
                        <li class="
...
<li class="
                        @if(!empty($view_type) &&  $view_type == 'customer_log')
                            active
                        @else
                            ''
                        @endif
                            ">
                            <a href="#customers_tab" data-toggle="tab" id="customer_tab" aria-expanded="true"><i class="fa fa-history" aria-hidden="true"></i> Logs</a>
                    </li>
...
@endif"
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <input type="hidden" class="req_from" value="profile_page"/>
                            ...
                            <div class="tab-pane
                                ...
                                @include('contact.partials.purchase_return_table')
                            </div>
                            <div class="tab-pane
                            @if(!empty($view_type) &&  $view_type == 'stock_report')
                                    active
...
@endif" id="payments_tab">
                        ...
                        @else
                                                        ''
                        @endif" id="followups_tab">
                            <div id="followup_div" style="height: 500px;overflow-y: scroll;"></div>
                    </div>
                        <div class="tab-pane
                        @if(!empty($view_type) &&  $view_type == 'customer_log')
                                active
@else
                                ''
@endif" id="customers_tab">
                            <div id="customer_div" style="height: 500px;overflow-y: scroll;"></div>
                        </div>
                        @if( in_array($contact->type, ['customer', 'both']) && session('business.enable_rp'))
                            <div class="tab-pane
                            @if(!empty($view_type) &&  $view_type == 'reward_point')
...
@endif"
    ...
    <div class="modal fade reconcile_payment_modal" tabindex="-1" role="dialog"
         aria-labelledby="gridSystemModalLabel">
    </div>
@stop
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            // automatically open select dropdown after delay
            // setTimeout(() => {
            ...
            supplier_stock_report_table = $('#supplier_stock_report_table').DataTable({
                ...
                columns: [
                    ...
                    {data: 'purchase_quantity', name: 'purchase_quantity', searchable: false, visible : false},
                    {data: 'total_quantity_sold', name: 'total_quantity_sold', searchable: false, visible : false},
                    {data: 'total_quantity_returned', name: 'total_quantity_returned', searchable: false, visible : false},
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    for (var r in data){
                        footer_stock += $(data[r].current_stock).data('orig-value') ? parseFloat($(data[r].current_stock).data('orig-value')) : 0;
                    }

                    $('.stock').html(footer_stock);
                },
                fnDrawCallback: function (oSettings) {
                    var footer_stock_price = sum_table_col($('#supplier_stock_report_table'), 'stock-price');
                    $('.stock_price').text(footer_stock_price);
        ...
        $(document).one('shown.bs.tab', 'a[href="#payments_tab"]', function () {
         ...
         $(document).one('shown.bs.tab', 'a[href="#followups_tab"]', function () {
            get_followup_deatils();
        });

         $(document).one('shown.bs.tab', 'a[href="#customers_tab"]', function () {
            get_customer_log();
        });
           ...
           var customer_id = $("#customer_id").val();
            $.ajax({
                ...
                success: function (result) {
                    $('.purchase_return')
                        .html(result);
                    __currency_convert_recursively($('.purchase_return'));
        ...
        function get_followup_deatils(){
            $.ajax({
                url: '{{action('ContactController@getFollowUpDetails', [$contact->id])}}',
                dataType: 'html',
                success: function (result) {
                    $('#followup_div').fadeOut(400, function () {
                        $('#followup_div')
                            .html(result).fadeIn(400);
                    });
        ...
        function get_customer_log(){
            $.ajax({
                url: "{{action('ContactController@getContactLogs', [$contact->id])}}",
                dataType: 'html',
                success: function (result) {
                    $('#customer_div').fadeOut(400, function () {
                        $('#customer_div')
                            .html(result).fadeIn(400);
                    });
